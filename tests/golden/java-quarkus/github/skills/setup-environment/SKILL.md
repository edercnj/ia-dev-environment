...
Manages the local development environment lifecycle for my-quarkus-service. Handles starting/stopping the container orchestrator, provisioning dependencies, building the application with maven, and verifying health checks.
...
- Build tool (maven) installed for application builds
...
5. **Rebuild** (`--build`):
   - Rebuild application using maven
...
```
## Environment Status -- my-quarkus-service
