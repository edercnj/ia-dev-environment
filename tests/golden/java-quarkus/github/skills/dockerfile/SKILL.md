...
Provides production-grade Dockerfile patterns for my-quarkus-service using java 21. Covers multi-stage builds for minimal image size, security hardening with non-root users, layer optimization for fast builds, health check configuration, OCI labels, and .dockerignore templates.
...
- maven configured for the project
- Understanding of java runtime requirements
...
4. **Configure health checks** -- Add HEALTHCHECK instruction:
   - Set appropriate timing based on java startup time
...
```
## Dockerfile Review -- my-quarkus-service
