...
Provides production-grade Kubernetes deployment patterns for my-fastapi-service. Covers workload type selection, pod specifications, resource sizing for python runtime, probe configuration, autoscaling, network policies, and security contexts using vanilla Kubernetes (cloud-agnostic).
...
2. **Configure pod specification** -- Define container spec:
   - Set resource requests and limits based on python runtime
...
- [ ] Resource requests and limits are set (based on python sizing guide)
...
```
## Kubernetes Deployment Review -- my-fastapi-service
