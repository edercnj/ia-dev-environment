...
| **fastapi-patterns** | `/fastapi-patterns` | FastAPI-specific patterns: Depends() DI, SQLAlchemy/Tortoise ORM data access, Pydantic models, APIRouter, BaseSettings config, httpx async testing, uvicorn deployment. Internal reference for agents producing FastAPI code. |
