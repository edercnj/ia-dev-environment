...
Provides production-grade Kubernetes deployment patterns for my-rust-service. Covers workload type selection, pod specifications, resource sizing for rust runtime, probe configuration, autoscaling, network policies, and security contexts using vanilla Kubernetes (cloud-agnostic).
...
2. **Configure pod specification** -- Define container spec:
   - Set resource requests and limits based on rust runtime
...
- [ ] Resource requests and limits are set (based on rust sizing guide)
...
```
## Kubernetes Deployment Review -- my-rust-service
