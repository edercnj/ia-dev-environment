...
Manages the local development environment lifecycle for my-rust-service. Handles starting/stopping the container orchestrator, provisioning dependencies, building the application with cargo, and verifying health checks.
...
- Build tool (cargo) installed for application builds
- rust 2024 runtime available
...
5. **Rebuild** (`--build`):
   - Rebuild application using cargo
...
- [ ] Application builds successfully with cargo
...
```
## Environment Status -- my-rust-service
