...
Provides production-grade Dockerfile patterns for my-rust-service using rust 2024. Covers multi-stage builds for minimal image size, security hardening with non-root users, layer optimization for fast builds, health check configuration, OCI labels, and .dockerignore templates.
...
- cargo configured for the project
- Understanding of rust runtime requirements
...
4. **Configure health checks** -- Add HEALTHCHECK instruction:
   - Set appropriate timing based on rust startup time
...
```
## Dockerfile Review -- my-rust-service
