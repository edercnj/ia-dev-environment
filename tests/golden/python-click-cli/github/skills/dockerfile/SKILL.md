...
Provides production-grade Dockerfile patterns for my-cli-tool using python 3.9. Covers multi-stage builds for minimal image size, security hardening with non-root users, layer optimization for fast builds, health check configuration, OCI labels, and .dockerignore templates.
