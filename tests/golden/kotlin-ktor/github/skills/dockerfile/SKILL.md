...
Provides production-grade Dockerfile patterns for my-ktor-service using kotlin 2.0. Covers multi-stage builds for minimal image size, security hardening with non-root users, layer optimization for fast builds, health check configuration, OCI labels, and .dockerignore templates.
...
- Understanding of kotlin runtime requirements
...
4. **Configure health checks** -- Add HEALTHCHECK instruction:
   - Set appropriate timing based on kotlin startup time
...
```
## Dockerfile Review -- my-ktor-service
