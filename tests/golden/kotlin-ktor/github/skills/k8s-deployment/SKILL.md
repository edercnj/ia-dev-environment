...
Provides production-grade Kubernetes deployment patterns for my-ktor-service. Covers workload type selection, pod specifications, resource sizing for kotlin runtime, probe configuration, autoscaling, network policies, and security contexts using vanilla Kubernetes (cloud-agnostic).
...
2. **Configure pod specification** -- Define container spec:
   - Set resource requests and limits based on kotlin runtime
...
- [ ] Resource requests and limits are set (based on kotlin sizing guide)
...
```
## Kubernetes Deployment Review -- my-ktor-service
