...
Provides production-grade Kubernetes deployment patterns for my-nestjs-service. Covers workload type selection, pod specifications, resource sizing for typescript runtime, probe configuration, autoscaling, network policies, and security contexts using vanilla Kubernetes (cloud-agnostic).
...
2. **Configure pod specification** -- Define container spec:
   - Set resource requests and limits based on typescript runtime
...
- [ ] Resource requests and limits are set (based on typescript sizing guide)
...
```
## Kubernetes Deployment Review -- my-nestjs-service
