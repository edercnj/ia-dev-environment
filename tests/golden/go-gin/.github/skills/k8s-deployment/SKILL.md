...
Provides production-grade Kubernetes deployment patterns for my-go-service. Covers workload type selection, pod specifications, resource sizing for go runtime, probe configuration, autoscaling, network policies, and security contexts using vanilla Kubernetes (cloud-agnostic).
...
2. **Configure pod specification** -- Define container spec:
   - Set resource requests and limits based on go runtime
...
- [ ] Resource requests and limits are set (based on go sizing guide)
...
```
## Kubernetes Deployment Review -- my-go-service
