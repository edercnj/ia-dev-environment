...
Manages the local development environment lifecycle for my-go-service. Handles starting/stopping the container orchestrator, provisioning dependencies, building the application with go-mod, and verifying health checks.
...
- Build tool (go-mod) installed for application builds
- go 1.22 runtime available
...
2. **Start environment** (`--start`):
   ...
   - Build application using go-mod
...
5. **Rebuild** (`--build`):
   - Rebuild application using go-mod
...
- [ ] Application builds successfully with go-mod
...
```
## Environment Status -- my-go-service
