...
### Added
- **Integrity Gate Between Phases (story-0005-0006):** Extended `IntegrityGateEntry` with `branchCoverage` (optional number) and `regressionSource` (optional string). Added integrity gate section to `x-dev-epic-implement` SKILL.md with gate subagent prompt, regression diagnosis, gate result registration, and RULE-004 enforcement. 27 new tests. Golden files updated for all 8 profiles.
