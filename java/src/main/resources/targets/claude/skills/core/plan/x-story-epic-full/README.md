...
| **Invocation** | `/x-story-epic-full <spec-file>` |
| **Delegates to** | `/x-story-epic`, `/x-story-create`, `/x-story-map` |
...
```mermaid
flowchart TD
    START(["/x-story-epic-full spec-file"]) --> PA
    ...
    subgraph PB["Phase B -- Generate Epic"]
        PB1["Delegate to /x-story-epic"] --> PB2[Generate epic-XXXX.md]
    ...
    subgraph PD["Phase D -- Generate Implementation Map"]
        PD1["Delegate to /x-story-map"] --> PD2[Dependency matrix + phase diagram]
...
| B | Generate Epic | Determine epic number, extract rules table, build story index, define DoR/DoD, create Jira issue (optional) | `/x-story-epic` |
...
| D | Generate Implementation Map | Dependency matrix, phase diagram, critical path, Mermaid dependency graph, strategic observations | `/x-story-map` |
...
- [x-story-epic](../x-story-epic/SKILL.md) -- Epic generation from a spec
- [x-story-create](../x-story-create/SKILL.md) -- Individual story file generation
- [x-story-map](../x-story-map/SKILL.md) -- Implementation map with dependency analysis
- [x-dev-epic-implement](../x-dev-epic-implement/SKILL.md) -- Executes the stories produced by this skill
