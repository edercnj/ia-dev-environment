...
argument-hint: "TASK-XXXX-YYYY-NNN [--from-cycle N] [--dry-run] [--warn-only-coverage]"
---
...
- `/x-test-tdd TASK-0029-0001-001 --warn-only-coverage` -- legacy mode (EPIC-0042): coverage gaps emit warning instead of blocking the cycle
...
| `--dry-run` | No | `false` | List all cycles with descriptions and TPP levels without executing any code |
| `--warn-only-coverage` | No | `false` | **Legacy override (EPIC-0042).** When set, coverage below 95% line / 90% branch emits a WARNING and the cycle still advances to REFACTOR. Default (flag absent) is the EPIC-0042 blocking behavior: sub-threshold coverage is an ERROR and BLOCKS the cycle. Use only for gradual migration of legacy tasks. |
