---
name: x-tdd
description: "Executes systematic Red-Green-Refactor TDD cycles for a task. Reads the task plan generated by x-plan-task, runs each cycle in TPP order, validates RED/GREEN/REFACTOR phases, delegates atomic commits to x-commit with TDD tags, and supports resume and dry-run."
user-invocable: true
...
Executes systematic Red-Green-Refactor TDD cycles for {{PROJECT_NAME}} tasks, ensuring every increment of code is tested, verified, and traceable. Reads the task plan generated by `/x-plan-task`, iterates TDD cycles in strict Transformation Priority Premise (TPP) order, validates each phase (RED must fail, GREEN must pass with minimum code, REFACTOR must preserve behavior), and delegates atomic commits to `/x-commit` with TDD tags (`[TDD:RED]`, `[TDD:GREEN]`, `[TDD:REFACTOR]`).
...
- `/x-tdd TASK-0029-0001-001` -- execute all TDD cycles for the task
- `/x-tdd TASK-0029-0001-001 --from-cycle 3` -- resume from cycle 3
- `/x-tdd TASK-0029-0001-001 --dry-run` -- list cycles without executing
...
```
x-tdd dry-run for TASK-XXXX-YYYY-NNN
...
5. **Validate RED**:
   ...
   - If the test **PASSES** (unexpected): **ABORT** with message:
     ...
     ```
6. Stage test files and delegate commit to `/x-commit`:
   ```
...
6. Stage implementation files and delegate commit to `/x-commit`:
   ```
   /x-commit --task TASK-XXXX-YYYY-NNN --type feat --subject "implement [description]" --tdd GREEN
   ```
...
3. If refactoring opportunities exist:
   ...
   d. Stage refactored files and delegate commit to `/x-commit`:
      ```
      /x-commit --task TASK-XXXX-YYYY-NNN --type refactor --subject "[refactoring description]" --tdd REFACTOR
      ```
...
```
x-tdd execution complete for TASK-XXXX-YYYY-NNN
...
```
OUTER LOOP (Acceptance Test -- driven by x-dev-lifecycle):
  Write failing acceptance test (end-to-end scenario)
  |
  INNER LOOP (Unit Tests -- driven by x-tdd):
...
- The outer loop is managed by `x-dev-lifecycle` or `x-dev-implement`
- This skill (`x-tdd`) drives the inner loop: systematic unit-level TDD cycles
...
| x-commit fails | ABORT: `"Commit failed at Cycle N, phase [RED|GREEN|REFACTOR]: {error}"` |
