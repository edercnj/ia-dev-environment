...
### Per-subagent telemetry markers (story-0040-0007 §3.2)

Before the single dispatch message that launches all 5 parallel subagents, emit
one `subagent-start` marker per role. After all 5 subagents return, emit the
matching `subagent-end` markers. The role names (Architect, QA, Security,
TechLead, PO) match the subagent sections below. These markers allow the
`/x-telemetry-analyze` report (EPIC-0040) to compute the overlap window and
identify consistently slow agents that bottleneck the parallel planning wave.
...
### Close subagent telemetry markers (emitted after all 5 subagents return)
...
<!-- TELEMETRY: phase.start -->
Bash command: `$CLAUDE_PROJECT_DIR/.claude/hooks/telemetry-phase.sh start x-story-plan Phase-4-DoR-Validation`
...
<!-- TELEMETRY: phase.end -->
Bash command: `$CLAUDE_PROJECT_DIR/.claude/hooks/telemetry-phase.sh end x-story-plan Phase-4-DoR-Validation ok`
