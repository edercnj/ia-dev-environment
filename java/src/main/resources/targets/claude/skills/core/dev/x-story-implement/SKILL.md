...
2. Status finalize block (Story/Map, v2 happy path):
   ...
   - **Step 3.8.2 — Validate transition.** Call `LifecycleTransitionMatrix.validateOrThrow(current, LifecycleStatus.CONCLUIDA)`. The only valid predecessor is `EM_ANDAMENTO`. If the transition is rejected: exit `STATUS_SYNC_FAILED` with the invalid `{current -> CONCLUIDA}` pair in stderr (Rule 22 RULE-046-04).
