...
Provides production-grade Kustomize patterns for managing Kubernetes manifests for {project_name} across multiple environments (dev, staging, prod). Covers directory structure, base and overlay organization, strategic merge patches, JSON 6902 patches, components for cross-cutting concerns, and generators for ConfigMaps and Secrets.
