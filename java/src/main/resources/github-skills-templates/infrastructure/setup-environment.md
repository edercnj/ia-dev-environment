...
Manages the local development environment lifecycle for {project_name}. Handles starting/stopping the container orchestrator, provisioning dependencies, building the application with {build_tool}, and verifying health checks.

**Condition**: This skill applies when orchestrator is not "none".

## Prerequisites

- Container runtime ({container}) installed and accessible in PATH
- Orchestrator ({orchestrator}) installed and configured
- Build tool ({build_tool}) installed for application builds
- {language_name} {language_version} runtime available
...
2. **Start environment** (`--start`):
   ...
   - Build application using {build_tool}
...
5. **Rebuild** (`--build`):
   - Rebuild application using {build_tool}
...
- [ ] Container runtime ({container}) is running
- [ ] Orchestrator ({orchestrator}) is accessible
- [ ] Application builds successfully with {build_tool}
...
```
## Environment Status -- {project_name}
