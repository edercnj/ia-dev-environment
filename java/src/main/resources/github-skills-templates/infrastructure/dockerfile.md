...
Provides production-grade Dockerfile patterns for {project_name} using {language_name} {language_version}. Covers multi-stage builds for minimal image size, security hardening with non-root users, layer optimization for fast builds, health check configuration, OCI labels, and .dockerignore templates.

**Condition**: This skill applies when container is not "none".

## Prerequisites

- Container runtime ({container}) installed and running
- Application source code with build configuration
- {build_tool} configured for the project
- Understanding of {language_name} runtime requirements
...
4. **Configure health checks** -- Add HEALTHCHECK instruction:
   - Set appropriate timing based on {language_name} startup time
...
```
## Dockerfile Review -- {project_name}
