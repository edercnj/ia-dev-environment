...
|------|-----------|----------|
| PRE_PLANNED | All 7 types exist AND `mtime(story) <= mtime(artifact)` for all | Skip Phase 1A-1F entirely. Phase 2 uses task-by-task execution. |
...
### Phase 2 — Task-by-Task Execution (PRE_PLANNED mode only)

When `planningMode == PRE_PLANNED`, Phase 2 replaces the standard implementation flow with task-by-task execution. All file references in this section are under `<EPIC_DIR>/plans/`:

1. Read `<EPIC_DIR>/plans/tasks-story-XXXX-YYYY.md` to get the ordered task list
2. For each TASK-NNN in dependency order:
   a. Read `<EPIC_DIR>/plans/task-plan-TASK-NNN-story-XXXX-YYYY.md` for Implementation Guide and DoD
   b. Execute the Implementation Guide instructions
   c. Validate DoD criteria
   d. Make atomic commit: `feat(scope): implement TASK-NNN [TDD:RED|GREEN|REFACTOR]`
3. After all tasks: verify all acceptance tests pass

When `planningMode != PRE_PLANNED`, Phase 2 executes as currently defined (no change).
...
### Phase 8 — Per-Task Status Update (PRE_PLANNED mode only)

When `planningMode == PRE_PLANNED`:
1. Update Section 8.1 of the story file: mark each TASK-NNN as Concluída
2. Update `execution-state.json`: set `tasks.TASK-NNN.status = "DONE"` for each completed task
3. Log: `"Updated {N} task statuses in story file and execution-state"`

When `planningMode != PRE_PLANNED`: no change to existing Phase 8 behavior.
