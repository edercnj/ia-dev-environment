...
4. Check if architecture plan exists at `plans/epic-XXXX/plans/architecture-story-XXXX-YYYY.md`
   ...
   - If absent: Phase 1 will evaluate decision tree and invoke x-arch-plan
...
Invoke `/x-arch-plan {STORY_PATH}` if Full or Simplified.
...
If `x-arch-plan` is not available, combine into Step 1B subagent.
...
- Invokes: `x-test-tdd` (Phase 2), `x-git-commit` (via x-test-tdd), `x-pr-create` (Phase 2), `x-code-format`/`x-code-lint` (via x-git-commit chain), `x-task-plan` (Phase 1), `x-arch-plan` (Phase 1), `x-test-plan`, `x-lib-task-decomposer`, `x-review` (Phase 3), `x-review-pr` (Phase 3), `x-arch-update` (Phase 3)
...
- `.github/skills/x-story-implement/SKILL.md`
- `.github/skills/x-task-implement/SKILL.md`
