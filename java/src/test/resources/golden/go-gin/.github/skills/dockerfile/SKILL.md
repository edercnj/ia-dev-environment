...
Provides production-grade Dockerfile patterns for my-go-service using go 1.22. Covers multi-stage builds for minimal image size, security hardening with non-root users, layer optimization for fast builds, health check configuration, OCI labels, and .dockerignore templates.
...
- go-mod configured for the project
- Understanding of go runtime requirements
...
4. **Configure health checks** -- Add HEALTHCHECK instruction:
   - Set appropriate timing based on go startup time
...
```
## Dockerfile Review -- my-go-service
