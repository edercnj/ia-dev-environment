...
description: >
  ...
  files), then delegates story execution to x-story-implement subagents.
---
...
| `--auto-approve-pr` | boolean | `false` | Propagate to x-story-implement: task PRs auto-merge into parent branches. Parent branches require human review. |
...
1. `plans/epic-XXXX/` directory exists — if not found, suggest `/x-epic-decompose`
2. `EPIC-XXXX.md` exists — if not found, suggest `/x-epic-create`
3. `IMPLEMENTATION-MAP.md` exists — if not found, suggest `/x-epic-map`
...
11. Delegate per-story execution to x-story-implement
...
- Subagent executes x-story-implement logic and returns `SubagentResult`
...
- Flag is propagated to each `x-story-implement` dispatch
...
- Invokes: `x-story-implement` (per-story), `x-epic-map` (error guidance)
