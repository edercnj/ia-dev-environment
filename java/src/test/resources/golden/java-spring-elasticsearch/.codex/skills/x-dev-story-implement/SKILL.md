         ...
         - If NOT --auto-approve-pr:
           Present AskUserQuestion with options:
             ...
             REJECT  -> task.status = FAILED, abort lifecycle with message:
                        "Task TASK-NNN rejected. Fix issues and resume with /x-dev-story-implement --task TASK-NNN"
...
1. Log: `"CONTEXT PRESSURE signal detected in x-dev-story-implement for {storyId}"`
...
**Error Reporting to Epic Orchestrator:** When `x-dev-story-implement` is invoked as a subagent by `x-dev-epic-implement`, errors MUST be reported back via the `SubagentResult` JSON fields: `errorType`, `errorMessage`, and `errorCode`.
