...
Provides production-grade Dockerfile patterns for ia-dev-environment using typescript 5. Covers multi-stage builds for minimal image size, security hardening with non-root users, layer optimization for fast builds, health check configuration, OCI labels, and .dockerignore templates.
...
- npm configured for the project
- Understanding of typescript runtime requirements
...
4. **Configure health checks** -- Add HEALTHCHECK instruction:
   - Set appropriate timing based on typescript startup time
...
```
## Dockerfile Review -- ia-dev-environment
