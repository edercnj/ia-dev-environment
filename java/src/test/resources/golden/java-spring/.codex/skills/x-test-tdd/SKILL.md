...
6. Stage implementation files and delegate commit to `/x-git-commit`:
   ```
   /x-git-commit --task TASK-XXXX-YYYY-NNN --type feat --subject "implement [description]" --tdd GREEN
   ```
...
3. If refactoring opportunities exist:
   ...
   d. Stage refactored files and delegate commit to `/x-git-commit`:
      ```
      /x-git-commit --task TASK-XXXX-YYYY-NNN --type refactor --subject "[refactoring description]" --tdd REFACTOR
      ```
...
Each phase delegates to `/x-git-commit`: read only the "## Slim Mode" section of x-git-commit for minimum context.
