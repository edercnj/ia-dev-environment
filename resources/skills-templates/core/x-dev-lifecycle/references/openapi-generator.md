...
- **Controllers** (e.g., `@Controller`, `@RestController` in {framework_name})
...
| `info.title` | `{project_name} API` | Mandatory |
...
Map {language_name} DTOs to JSON Schema types:

| {language_name} Type | JSON Schema Type |
|----------------------|------------------|
| `string` | `type: "string"` |
