...
Provides production-grade Kubernetes deployment patterns for {project_name}. Covers workload type selection, pod specifications, resource sizing for {language_name} runtime, probe configuration, autoscaling, network policies, and security contexts using vanilla Kubernetes (cloud-agnostic).
...
2. **Configure pod specification** -- Define container spec:
   - Set resource requests and limits based on {language_name} runtime
...
- [ ] Resource requests and limits are set (based on {language_name} sizing guide)
...
```
## Kubernetes Deployment Review -- {project_name}
