# História: README e Validação End-to-End da estrutura Copilot
...
| RULE-007 | Paridade de checkpoints entre hooks .claude e .github |

## 3. Descrição

Como **QA Engineer**, eu quero documentar a estrutura `.github/` e executar validação transversal final, garantindo que a adoção ocorra com evidências de conformidade e operação.

Produz README de governança com árvore, convenções, fluxos e manutenção.

Executa validação end-to-end de instructions, skills, agents, prompts, hooks e MCP com relatório Go/No-Go.

### 3.1 Governança documental

- README na raiz de `.github/`.
- Conveções e mapeamentos explicitados.
- Links relativos para conteúdo de referência.

### 3.2 Validação final

- Checks de estrutura e frontmatter.
- Smoke de workflows críticos.
- Relatório final com severidade de achados.
...
| :--- | :--- | :--- | :--- | :--- |
| `check_id` | string | M | - | Identificador do check |
| `scope` | enum(instructions,skills,agents,prompts,hooks,mcp) | M | - | Domínio validado |
| `expected_result` | string | M | - | Critério de sucesso |
| `severity` | enum(critical,major,minor) | M | - | Impacto do desvio |
...
```gherkin
Cenario: Fluxo de sucesso de STORY-007
...
Cenario: Violação de regra transversal em STORY-007
...
Cenario: Input malformado em STORY-007
...
Cenario: Edge case de concorrência/ordem em STORY-007
