# História: Prompts de Composição de Workflows (.prompt.md)
...
| Blocked By | Blocks |
| :--- | :--- |
| STORY-002, STORY-003, STORY-004 | STORY-007 |
...
Como **Product Owner Técnico**, eu quero criar prompts para workflows completos (`new-feature`, `decompose-spec`, `code-review`, `troubleshoot`), garantindo que times executem processos completos com menor fricção operacional.

Prompts orquestram skills e agents, conectando entradas (spec, PR, erro) a saídas concretas.

Cada prompt deve declarar quando usar e quais capacidades compõe, sem duplicar conteúdo das skills.

### 3.1 Pacote de prompts

- Criar 4 arquivos `.prompt.md` prioritários.
- Frontmatter válido em todos.
- Instruções orientadas a resultado.

### 3.2 Orquestração

- Dependências explícitas de skills/agentes.
- Fallback documentado para indisponibilidade parcial.
- Reuso de templates já existentes.
...
| :--- | :--- | :--- | :--- | :--- |
| `prompt_name` | string | M | - | Identificador do prompt |
| `orchestrated_skills` | array[string] | M | - | Skills chamadas pelo fluxo |
| `required_agents` | array[string] | O | - | Agentes especializados |
| `expected_outputs` | array[path] | M | - | Artefatos gerados |
...
```gherkin
Cenario: Fluxo de sucesso de STORY-005
...
Cenario: Violação de regra transversal em STORY-005
...
Cenario: Input malformado em STORY-005
...
Cenario: Edge case de concorrência/ordem em STORY-005
