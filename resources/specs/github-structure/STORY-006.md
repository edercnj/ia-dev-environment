# História: Cross-cutting Config: Hooks JSON + MCP
...
Como **DevOps Engineer**, eu quero implementar `.github/hooks/*.json` e `.github/copilot-mcp.json` com governança, garantindo que o runtime tenha automações e integrações determinísticas.

Cobre hooks prioritários (`post-compile-check`, `pre-commit-lint`, `session-context-loader`) em JSON.

Inclui configuração MCP versionada para integrações externas sem expor segredos.

### 3.1 Hooks em JSON

- Eventos e matchers compatíveis com Copilot.
- Comando, timeout e descrição obrigatórios.
- Paridade com checkpoints da `.claude/`.

### 3.2 Configuração MCP

- Schema claro para servidores MCP.
- Capacidades e env keys documentadas.
- Validação de parse antes da entrega.
...
| :--- | :--- | :--- | :--- | :--- |
| `hook_event` | enum(sessionStart,preToolUse,postToolUse) | M | - | Evento de gatilho |
| `matcher` | object | O | - | Filtro de ferramenta/evento |
| `command` | string | M | - | Comando associado ao hook |
| `mcp_server_id` | string | O | - | Servidor MCP configurado |
...
```gherkin
Cenario: Fluxo de sucesso de STORY-006
...
Cenario: Violação de regra transversal em STORY-006
...
Cenario: Input malformado em STORY-006
...
Cenario: Edge case de concorrência/ordem em STORY-006
