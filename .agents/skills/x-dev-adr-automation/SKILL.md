...
```bash
# Example: search for mini-ADR markers
...
1. **Scan**: Use `Glob` to find all files matching `docs/adr/ADR-*.md`
...
- Works with any project — the `docs/adr/` directory is created if it does not exist
