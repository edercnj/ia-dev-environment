...
| STORY-014 | GitHub Assemblers | STORY-005, STORY-006, STORY-008 | STORY-016, STORY-021 | Pendente |
| STORY-015 | ReadmeAssembler | STORY-005, STORY-008 | STORY-016 | Pendente |
| STORY-021 | Lib Skills — GitHub Skills Assembler | STORY-014 | STORY-016 | Pendente |
| STORY-016 | Pipeline Orchestrator | STORY-002, STORY-009..015, STORY-021 | STORY-018 | Pendente |
...
> **Nota:** STORY-016 é o ponto de convergência principal — depende de todos os assemblers (009-015), do módulo de exceptions/utils (002), e de STORY-021 (Lib Skills Enhancement). STORY-018 é o segundo ponto de convergência, integrando pipeline, config loader e interactive mode.
...
╚════════╪═════════════╪═════════════╪═════════════════════════════════╝
         ...
         ▼             ▼             ▼
╔══════════════════════════════════════════════════════════════════════════╗
║              FASE 5 — Lib Skills Enhancement (1 story)                 ║
║                                                                        ║
║   ┌──────────────────────────────────────────────────────────┐         ║
║   │  STORY-021  Lib Skills — GitHub Skills Assembler          │         ║
║   │  (← STORY-014)                                            │         ║
║   └──────────────────────────┬───────────────────────────────┘         ║
╚══════════════════════════════╪═════════════════════════════════════════╝
                               │
                               ▼
╔══════════════════════════════════════════════════════════════════════════╗
║                   FASE 6 — Pipeline Integration (1 story)              ║
║                                                                        ║
║   ┌──────────────────────────────────────────────────────────┐         ║
║   │  STORY-016  Pipeline Orchestrator                         │         ║
║   │  (← STORY-002, STORY-009..015, STORY-021)                 │         ║
║   └──────────────────────────┬───────────────────────────────┘         ║
╚══════════════════════════════╪═════════════════════════════════════════╝
                               │
                               ▼
...
```
STORY-001 ──→ STORY-003 ──→ STORY-006 ──→ STORY-007 ──→ STORY-014 ──→ STORY-021 ──→ STORY-016 ──→ STORY-018 ──→ STORY-019 ──→ STORY-020
  Fase 0         Fase 1         Fase 2         Fase 3         Fase 4         Fase 5          Fase 6         Fase 7         Fase 8         Fase 9
```

**10 fases no caminho crítico, 10 histórias na cadeia mais longa (STORY-001 → 003 → 006 → 007 → 014 → 021 → 016 → 018 → 019 → 020).**

Qualquer atraso no caminho crítico impacta diretamente a data de entrega final. As fases 4 (assemblers) e 1 (core primitives) oferecem o maior paralelismo para absorver variações. A Fase 5 (Lib Skills Enhancement) adiciona uma dependência serial entre STORY-014 e STORY-016.
...
graph TD
    ...
    S021["STORY-021<br/>Lib Skills GitHub"]
    ...
    classDef fase5 fill:#ff6b6b,stroke:#fff,color:#fff
    classDef fase6 fill:#e94560,stroke:#fff,color:#fff
    classDef fase7 fill:#6c5ce7,stroke:#a29bfe,color:#fff
    classDef fase8 fill:#d63031,stroke:#fff,color:#fff
    classDef fase9 fill:#00b894,stroke:#fff,color:#fff
...
| 5 | STORY-021 | Lib Skills Enhancement | 1 | STORY-014 (Fase 4) concluída |
| 6 | STORY-016 | Pipeline Integration | 1 | Fase 5 concluída + STORY-002 + STORY-009..015 |
| 7 | STORY-018 | CLI | 1 | Fase 6 + STORY-004 + STORY-017 |
| 8 | STORY-019 | Validation | 1 | Fase 7 concluída |
| 9 | STORY-020 | Release | 1 | Fase 8 concluída |

**Total: 21 histórias em 10 fases.**

> **Nota:** A Fase 4 oferece o maior potencial de paralelismo (7 stories simultâneas). STORY-017 (Interactive Mode) pode começar na Fase 3 pois depende apenas de STORY-003 e STORY-004, executando em paralelo com os assemblers. A Fase 5 (Lib Skills Enhancement) é uma dependência serial entre STORY-014 e STORY-016.
...
### Fase 5 — Lib Skills Enhancement

| Story | Escopo Principal | Artefatos Chave |
| :--- | :--- | :--- |
| STORY-021 | Lib skills no GithubSkillsAssembler | `resources/github-skills-templates/lib/`, `src/assembler/github-skills-assembler.ts` |

**Entregas da Fase 5:**

- 3 templates GitHub para lib skills criados
- Grupo "lib" adicionado ao SKILL_GROUPS
- Lib skills geradas em `.github/skills/lib/`
...
- **STORY-016 (Pipeline)** é o ponto de convergência mais complexo: depende de STORY-002 (Fase 1), todas as 7 stories de assembler (Fase 4), e STORY-021 (Fase 5). Só pode começar após a conclusão completa da Fase 5.
