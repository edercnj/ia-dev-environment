...
Parallel dispatch (SINGLE message):
  ├── Subagent 1: Interface doc generator (OpenAPI | gRPC | CLI | GraphQL | Event)
  ├── Subagent 2: Architecture doc update (if arch plan exists)
  └── Subagent 3: Performance baseline (if feature affects request path)
