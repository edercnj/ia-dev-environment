...
Após alterar os SKILL.md na source of truth (`java/src/main/resources/targets/claude/skills/`), regenerar (comando canônico do README §"Regenerating Golden Files"):
```bash
cd java
...
A validação de não-regressão deve usar a suíte atual executada em `mvn verify` e as convenções existentes referenciadas nesta story (Rules 13 e 22), sem depender de um teste chamado `LifecycleIntegrityAuditTest` ou de IDs de violação não confirmados no código atual — `LifecycleIntegrityAuditTest` é produto ainda em andamento do EPIC-0046 (story-0046-0007, "In progress") e não existe no branch `develop` no momento desta story. O retrofit não deve introduzir novas violações — apenas adicionar markers onde estão ausentes. Caso o EPIC-0046 conclua antes desta story, reavaliar e referenciar o teste real.
...
- **Métrica de Sucesso:** Grep por `MANDATORY TOOL CALL` nos 6 SKILL.md modificados retorna match em cada ponto de invocação crítica; `mvn verify` passa sem regressão (e, se já mergeado, o `LifecycleIntegrityAuditTest` do EPIC-0046 também).
...
- [ ] `mvn verify` passa sem novas violações (ou, se `LifecycleIntegrityAuditTest` já estiver disponível no `develop` quando a story for executada, ele também passa)
...
| :--- | :--- | :--- | :--- |
| `skills/core/dev/x-story-implement/SKILL.md` | ~linha 202 | `x-pr-watch-ci` | MANDATORY TOOL CALL |
| `skills/core/dev/x-task-implement/SKILL.md` | ~linha 496 | `x-pr-watch-ci` | MANDATORY TOOL CALL |
| `skills/core/ops/x-release/SKILL.md` | ~linha 1381 | `x-pr-watch-ci` | MANDATORY TOOL CALL |
| `skills/core/review/x-review/SKILL.md` | linhas 99–155 (9 especialistas) | specialists | MANDATORY TOOL CALL (×9) |
| `skills/core/dev/x-epic-implement/SKILL.md` | ~linha 333 | `x-pr-fix-epic` | MANDATORY TOOL CALL |
| `skills/core/security/x-owasp-scan/SKILL.md` | ~linha 273 | `x-dependency-audit` | MANDATORY TOOL CALL |
...
| 1 | `AUDIT_REGRESSION` | `mvn verify` detectou nova violação (ou `LifecycleIntegrityAuditTest`, se disponível) | Corrigir antes do merge |
...
sequenceDiagram
    ...
    participant Audit as mvn verify (+ LifecycleIntegrityAuditTest se disponível)
    ...
    Dev->>Audit: mvn verify
    Audit->>Golden: Escaneia skills por violações (conforme testes existentes)
...
Cenario: Marker adicionado e golden regenerado (happy path)
  ...
  E `mvn verify` passa sem novas violações (+ `LifecycleIntegrityAuditTest` se disponível)
...
- **Testability:** Config + VerificationTest
- **Files:**
  - `java/src/main/resources/targets/claude/skills/core/review/x-review/SKILL.md`
