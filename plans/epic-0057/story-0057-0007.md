...
Esta story toma a decisão e a implementa considerando o mecanismo real existente no `java/pom.xml`: smoke tests são atualmente controlados por **path include/exclude** (`**/smoke/**/*.java` — não por `@Tag("smoke")`). Opção A: **ajustar a configuração do surefire/failsafe no `pom.xml` para incluir os smoke tests selecionados também em `mvn test`** (remover a exclusão por path para o subconjunto promovido). Opção B: **criar hook Git `pre-push`** que executa `mvn test -Dtest='**/smoke/**'` (ou invoca um perfil equivalente) antes de qualquer push. A decisão entre as duas opções deve ser feita analisando impacto no tempo de build local. Caso a implementação escolha introduzir `@Tag("smoke")` como mecanismo secundário, o próprio `pom.xml` deve ser atualizado para reconhecer a tag via `groups` do surefire.
...
| Outros testes em `dev.iadev.smoke` (pasta `**/smoke/**`) | `dev.iadev.smoke` | variável | Apenas em `mvn verify` |
