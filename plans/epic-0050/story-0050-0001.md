...
- Numeração: **Rule 23**, próximo slot livre após Rule 22 (EPIC-0049 está usando 21 e 22). A numeração está comprometida com 23 em todos os demais artefatos deste épico (SPEC, stories, IMPLEMENTATION-MAP); alterações futuras exigem sincronizar todos.
- Source-of-truth: `java/src/main/resources/targets/claude/rules/23-model-selection.md`
- Output gerado: `.claude/rules/23-model-selection.md`
...
- **Métrica de Sucesso:** Arquivo gerado em `.claude/rules/23-model-selection.md` passa em todos os 17 stacks suportados; `CLAUDE.md` e `README.md` referenciam-no corretamente; audit script mencionado na seção "Audit contract" é executável e retorna exit 0 quando invocado em estado limpo.
...
- [ ] Numeração confirmada como **Rule 23** (já comprometida em todos os artefatos do épico)
...
- [ ] Arquivo `targets/claude/rules/23-model-selection.md` criado com as 8 seções obrigatórias
...
| :--- | :--- | :--- |
| Rule file | `.claude/rules/23-model-selection.md` | Markdown com as 8 seções listadas em 3.2 |
...
sequenceDiagram
    ...
    Dev->>Src: Escreve 23-model-selection.md
    Dev->>Src: Atualiza CLAUDE.md + README.md
    Dev->>Gen: mvn process-resources + regenerate
    Gen->>Out: Gera .claude/rules/23-model-selection.md
...
Cenário: Rule é gerada no output
  ...
  ENTÃO o output contém ".claude/rules/23-model-selection.md"
...
Cenário: CLAUDE.md referencia a nova Rule
  ...
  ENTÃO contém uma linha para "23-model-selection"
...
- **Files:**
  - `java/src/main/resources/targets/claude/rules/23-model-selection.md` (novo)
- **Acceptance Criteria:**
...
- **Acceptance Criteria:**
  - [ ] Tabela de rules em CLAUDE.md inclui linha para Rule 23
...
- **Acceptance Criteria:**
  ...
  - [ ] Smoke: ler `.claude/rules/23-model-selection.md` em output e validar seções via grep
