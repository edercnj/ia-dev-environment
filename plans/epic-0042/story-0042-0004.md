...
- **Gating:** executa SOMENTE se a decisão do Tech Lead, lida/parseada do artefato `review-tech-lead-story-XXXX-YYYY.md` gerado por `x-review-pr` (ou do dashboard derivado dele), for `GO`. Em `NO-GO`, pula o Step 3.6.5 e o fluxo reentra no Step 3.5 para remediação.
- **Discovery:** lista os PRs de task a partir de `stories[storyId].tasks[*].prNumber` no `execution-state.json` da story (ou do estado consolidado no epic pai, quando aplicável). O Step 3.6.5 deve considerar apenas `prNumber` não-nulo; tasks com `prNumber = null` são tratadas como "sem PR de task associado" e não entram na varredura de PRs.
- **Per-PR loop (serial):** para cada task com `prNumber` não-nulo, checa a quantidade de **review comments** via `gh api repos/{owner}/{repo}/pulls/{PR}/comments` (contando o array retornado), alinhado ao mesmo tipo de comentário consumido pelo `x-pr-fix`. PRs com `reviewComments > 0` recebem `Skill(skill: "x-pr-fix", args: "<pr>")` via INLINE-SKILL. PRs com `reviewComments = 0` são logados como `SKIP` e ignorados. Tasks com `prNumber = null` também são logadas como `SKIP/NO_PR`.
...
- **Idempotência:** o Step 3.6.5 roda EXATAMENTE UMA VEZ por ciclo de story. Uma reentrada no Step 3.5 após `NO-GO` e posterior novo `GO` re-dispara o Step 3.6.5 apenas se o Step 3.6 foi re-executado.
...
> **Nota sobre contrato de saída:** O `x-pr-fix` atual emite apenas um relatório Markdown (tabela de comentários + ações) e não promete os campos acima como JSON parseável. A implementação do Step 3.6.5 deve adotar uma de duas estratégias: (a) estender `x-pr-fix` para emitir um bloco JSON estável no final do output (preferível) ou (b) detectar regressão de compilação indiretamente executando `mvn compile` após cada invocação de `x-pr-fix` e tratando exit != 0 como `failed > 0`. A decisão deve ser tomada na fase de implementação da story.
...
| :--- | :--- | :--- |
| `PR_FIX_COMPILE_REGRESSION` | `x-pr-fix` retornou `failed > 0` com razão compile | `PR #N: fix automático quebrou compilação. Step 3.7 abortado. Intervenção manual necessária.` |
...
sequenceDiagram
    ...
    loop Para cada task PR
        Story->>GH: gh api repos/{owner}/{repo}/pulls/{pr}/comments (count array)
        alt comments > 0
...
- **Acceptance Criteria:**
  - [x] Seção "Step 3.6.5 — Auto-Fix Task PR Comments (post-TL GO)" inserida entre o fim do Step 3.6 e o início do Step 3.7
  - [x] Gating (TL=GO) documentado como primeira linha acionável
  - [x] Discovery via `gh pr view <pr> --json comments --jq '.comments | length'` documentada
  - [x] Per-PR loop com `Skill(skill: "x-pr-fix", args: "<pr>")` — Rule 13 Pattern 1
  - [x] Compile-regression guard com `PR_FIX_COMPILE_REGRESSION`
  - [x] Nota explícita de idempotência (single-pass) alinhada com RULE-007
  - [x] Zero bare-slash em delegação (audit Rule 13 green)
...
- **Files:**
  - `java/src/main/resources/targets/claude/skills/core/dev/x-story-implement/SKILL.md`
- **Acceptance Criteria:**
  - [x] Nova linha na tabela Integration Notes: `x-pr-fix | Invokes (Step 3.6.5, on TL GO) | Per-task PR comment auto-fix after Tech Lead approval`
  - [x] Linha inserida em ordem alfabética ou na posição canônica da tabela
...
- **Files:**
  - `java/src/main/resources/targets/claude/skills/core/dev/x-story-implement/SKILL.md`
- **Acceptance Criteria:**
  - [x] Nova linha na tabela Error Handling: `x-pr-fix returns compile regression | Abort Step 3.6.5 with PR_FIX_COMPILE_REGRESSION; do NOT proceed to Step 3.7`
  - [x] Linha inserida sem afetar ordem das linhas existentes
...
- **Acceptance Criteria:**
  - [x] `mvn process-resources` executado; `.claude/skills/x-story-implement/SKILL.md` atualizado
  - [x] Golden regenerado via comando canônico (README.md:810-818)
  - [x] `git diff src/test/resources/golden/claude/skills/x-story-implement/SKILL.md` tem o escopo contido na região entre Step 3.6 e Step 3.7 (além das linhas adicionadas em Integration Notes e Error Handling)
  - [x] `mvn test -Dtest=*GoldenDiff*` verde
  - [x] Audit Rule 13 green (bare-slash em contexto de delegação = 0)
