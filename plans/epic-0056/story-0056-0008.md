...
1. Executar o modo de regeneração canônico: `bash java/install.sh --dev` OU diretamente `mvn -f java/pom.xml exec:java -Dexec.mainClass=dev.iadev.golden.GoldenFileRegenerator -Dexec.classpathScope=test`. Não existe flag `--regenerate-goldens` na CLI `ia-dev-env`.
...
sequenceDiagram
    ...
    Dev->>Gen: mvn exec:java -Dexec.mainClass=dev.iadev.golden.GoldenFileRegenerator
