...
Esta é a fundação do épico: todas as outras stories consomem as regras definidas aqui. O KP precisa usar `user-invocable: false` como mecanismo de classificação de knowledge pack interno (x-lib-*/KP), de modo que não seja exposto no menu `/`, e ficar sob a subpasta `plan/` da taxonomia de skills (EPIC-0036).
...
Cenario: Frontmatter inválido (error path)
  DADO que o SKILL.md tem `user-invocable: true` (classifica como skill invocável, não KP)
