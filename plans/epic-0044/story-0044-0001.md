...
`StackMapping` era originalmente o hub de mapeamento de settings de stack. Epic-0023 introduziu `DatabaseSettingsMapping` como proprietário canônico das tabelas de database e cache settings e marcou as entradas equivalentes em `StackMapping` como deprecated `forRemoval`. A migração parou no meio: `DatabaseSettingsMapping` já é consumido em vários pontos (incluindo `RulesConditionals.java`, já totalmente migrado), mas 1 arquivo de produção (`PermissionCollector.java`) e 9+ arquivos de teste continuam chamando os símbolos legados. Esta história conclui a migração remanescente e remove os 4 símbolos.
...
### 3.1 Migração de `PermissionCollector.java`
...
- Import: ajustar conforme necessário (note que `PermissionCollector.java:79` também chama `StackMapping.getSettingsLangKey(...)`, que NÃO é deprecated e permanece inalterado — manter o import de `StackMapping` por causa dessa chamada).
...
- [ ] Worktree criado em `.claude/worktrees/story-0044-0001/` a partir de branch `feat/story-0044-0001-remove-stackmapping-deprecated` (Rule 14).

### DoD Local (Definition of Done)

- [ ] 4 símbolos deprecated removidos de `StackMapping.java:207–217`.
- [ ] `PermissionCollector.java:122,128` migrado para `DatabaseSettingsMapping`.
- [ ] Testes citados em 3.2 migrados ou removidos com cobertura preservada via `DatabaseSettingsMappingTest`.
...
- [ ] PR aberto targeting `develop` com título `refactor(story-0044-0001): remove deprecated StackMapping symbols` (RULE-005).
...
sequenceDiagram
    ...
    participant Caller as PermissionCollector
...
- A primeira transformação (walking skeleton) é trocar uma única linha em `PermissionCollector.java:122` — menor diff possível que compila.
...
### TASK-0044-0001-001: Migrar `PermissionCollector.java` para `DatabaseSettingsMapping`
...
- **Files:**
  - `java/src/main/java/dev/iadev/application/assembler/PermissionCollector.java` (linhas 122, 128 — migrar; manter linha 79 `StackMapping.getSettingsLangKey(...)` intacta, pois não é deprecated)
  - `java/src/test/java/dev/iadev/application/assembler/PermissionCollectorTest.java` (atualizar referências se existirem)
- **Acceptance Criteria:**
  - [ ] `grep -rE "StackMapping\\.(DATABASE_SETTINGS_MAP|CACHE_SETTINGS_MAP|getDatabaseSettingsKey|getCacheSettingsKey)" java/src/main/java/dev/iadev/application/assembler/` retorna 0 matches.
  - [ ] `mvn compile -pl java` verde sem warnings `[removal]` no arquivo migrado.
  - [ ] Testes de `PermissionCollector` permanecem verdes.
