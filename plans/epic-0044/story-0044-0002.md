...
- **Acceptance Criteria:**
  - [x] 6 assemblers chamam apenas `resolveResourceDir(String)`.
  - [x] `mvn test -pl java -Dtest="Docs*,GrpcDocs*,Readme*,ReleaseChecklist*"` verde.
  - [x] Golden files byte-idênticos OU regenerados com diff justificado.
...
- **Acceptance Criteria:**
  - [x] 6 assemblers migrados.
  - [x] Suite de testes correspondente verde.
  - [x] Golden files consistentes.
...
- **Acceptance Criteria:**
  - [x] 6 assemblers migrados.
  - [x] `ConstitutionAssembler` validado com atenção (único `depth=4`).
  - [x] Golden files consistentes, com regeneração documentada se necessário.
...
- **Files:**
  ...
  - Respectivos `*Test.java`
- **Acceptance Criteria:**
  - [x] 5 assemblers migrados; `AssemblerFactory` é o último.
  - [x] `mvn verify` verde após esta task (todas as 23 migrações completas).
  - [x] `grep -r "resolveResourcesRoot" java/src/main/java/dev/iadev/application/assembler/` retorna 0 matches.
...
- **Acceptance Criteria:**
  - [x] `grep -r "resolveResourcesRoot" java/src/test/` retorna 0 matches.
  - [x] `mvn test -pl java` verde.
  - [x] Cobertura de `ResourceResolver` ≥ baseline (todos os paths de `resolveResourceDir` cobertos).
...
- **Acceptance Criteria:**
  - [x] `ResourceResolver.java` não contém `resolveResourcesRoot`.
  - [x] `mvn clean verify` verde com exit code 0.
  - [x] `mvn clean compile 2>&1 | grep "\\[removal\\]" | grep resolveResourcesRoot` retorna 0 linhas.
  - [x] `ResourceResolverDeprecatedRemovedTest` valida ausência via reflexão (espera `NoSuchMethodException` para os 2 overloads).
  - [x] `CHANGELOG.md` atualizado conforme RULE-008.
  - [x] Commit de remoção usa prefixo `refactor(story-0044-0002):` conforme RULE-005.
