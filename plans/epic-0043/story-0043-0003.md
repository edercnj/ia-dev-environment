...
- Default: menu com PROCEED (aprova, avança Phase 1) / REJECT (retorna para Step 0.5.2 com feedback) / ABORT (encerra)
- Nota: este gate não tem um PR para "fixar", portanto a 2ª opção é **REJECT** em vez de **FIX-PR** (desvio autorizado pela Rule 20: gates sem PR associado podem usar REJECT como sinônimo semântico do loop-back, indicando "volta ao passo anterior para regenerar")
...
| `tasks[TASK-ID].lastGateDecision` | `Enum` | O (novo) | `PROCEED` \| `REJECT` \| `FIX_PR` \| `ABORT` \| `FORCE_PROCEED` | `"FIX_PR"` |
...
| :--- | :--- | :--- |
| `STORY_TASK_FIX_LOOP_EXCEEDED` | 3 FIX-PR consecutivos em uma task | `"Loop de fix excedeu 3 tentativas na task ${TASK_ID}; apresentando opção FORCE-PROCEED"` |
...
stateDiagram-v2
    ...
    MENU_229 --> FORCE_PROCEED: após 3 fixes

    FIX_PR --> XFIX: Skill(skill: "x-pr-fix", args: "<PR>")
    XFIX --> MENU_229: loop-back

    PROCEED --> NEXT_TASK
    FORCE_PROCEED --> NEXT_TASK
...
- **Acceptance Criteria:**
  ...
  - [ ] 3 fixes consecutivos → FORCE-PROCEED aparece
