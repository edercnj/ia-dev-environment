...
- `/x-release 3.7.0 --non-interactive` → comportamento idêntico ao HALT textual atual (para CI/automação)
- `/x-release 3.7.0 --interactive` → emite warning `DEPRECATED_FLAG: --interactive é default agora; comportamento inalterado` e prossegue com menu
- `/x-release 3.7.0 --continue-after-merge` → mantido (entrypoint para resume pós-merge via automação)
...
- 3 FIX-PR consecutivos → menu passa a mostrar 4ª opção `FORCE-PROCEED`
...
- [ ] `x-release/SKILL.md` Phase 8 reescrita: menu default; `--non-interactive` documentado; `--interactive` marcado DEPRECATED com warning
...
- [ ] State file schema estendido com `lastGateDecision` e `fixAttempts[]`; migração silenciosa para files antigos
- [ ] Guard-rail de 3 FIX-PR consecutivos → FORCE-PROCEED documentado
...
| `lastGateDecision` | `Enum` | O (novo) | `PROCEED` \| `FIX_PR` \| `ABORT` \| `FORCE_PROCEED` | `"FIX_PR"` |
| `fixAttempts` | `List<FixAttempt>` | O (novo, default `[]`) | ≤ 3 antes de FORCE-PROCEED | ver Rule 20 §5.1 |
...
| :--- | :--- | :--- |
| `RELEASE_FIX_LOOP_EXCEEDED` | 3 `FIX_PR` consecutivos sem `PROCEED` intermediário | `"Loop de fix excedeu 3 tentativas no release ${VERSION}; apresentando opção FORCE-PROCEED"` |
...
sequenceDiagram
    ...
    alt decisão == FIX_PR
        ...
        alt fixAttempts.size < 3
            ...
            Release->>AUQ: menu + FORCE-PROCEED
        end
    ...
    else decisão == FORCE_PROCEED (após 3 fixes)
        Release->>State: log elevado
        Release->>Release: avança Phases 9-12
    end
```
...
Cenario: Boundary - 3 FIX-PR consecutivos ativam FORCE-PROCEED
  DADO release em Phase 8 e ja ocorreram 2 FIX-PR consecutivos
  QUANDO operador seleciona FIX-PR novamente (3a tentativa)
  ENTAO apos retorno de x-pr-fix menu apresenta 4 opcoes incluindo FORCE-PROCEED
...
Cenario: Error - --interactive deprecado mas funcional
  DADO release invocado com --interactive
  QUANDO Phase 8 executa
  ENTAO warning DEPRECATED_FLAG e impresso uma unica vez
  E menu e exibido normalmente (comportamento igual ao default)
...
Degenerate (menu default sem state) → Happy PROCEED → Happy FIX-PR 1x → Boundary 3x → Error `--non-interactive` → Error `--interactive` deprecated → Boundary legacy state.
...
- **Acceptance Criteria:**
  ...
  - [ ] `--non-interactive` preserva HALT textual legado para CI
  - [ ] `--interactive` flag documentada como DEPRECATED com warning
  - [ ] FIX-PR handler: Pattern 1 INLINE-SKILL com args `<PR_NUMBER>`
  - [ ] Loop-back após retorno de `x-pr-fix` documentado
  - [ ] 3 FIX-PR consecutivos → 4ª opção FORCE-PROCEED documentada
...
- **Outputs:**
  ...
  - Arquivo contém diagrama Mermaid com 3 opções + FORCE-PROCEED (verificação: `grep -q "FORCE_PROCEED" java/src/main/resources/targets/claude/skills/core/ops/x-release/references/approval-gate-workflow.md`)
- **Acceptance Criteria:**
  - [ ] Seção principal reescrita para comportamento novo (menu default)
