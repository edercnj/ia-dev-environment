...
- **Diretório alvo:** `java/src/main/resources/targets/claude/skills/core/`
- **Arquivos:** apenas `SKILL.md` e `references/*.md`
- **Exceções (allowlist):** seções `## Triggers` e `## Examples` onde o bare-slash e o nome textual do flag são legítimos (usuário tipando no chat)
- **Regex 1 (HALT textual sem menu):** arquivos que contêm a palavra `HALT` em contexto de pause/exit sem `AskUserQuestion` no mesmo bloco (janela de ~30 linhas)
- **Regex 2 (flags depreciadas):** matches de `\\-\\-interactive\\b`, `\\-\\-manual-task-approval\\b`, `\\-\\-manual-contract-approval\\b`, `\\-\\-manual-batch-approval\\b` em contexto de código/delegação (não prose)
...
Degenerate (repo limpo) → Happy baseline → Error injetado HALT → Error flag depreciada → Boundary Triggers allowlist → Boundary grep ausente.
...
- **Acceptance Criteria:**
  ...
  - [ ] Regex patterns 1 e 2 implementados via grep
...
- **Acceptance Criteria:**
  ...
  - [ ] Naming segue convenção `[method]_[scenario]_[expectedBehavior]`
