...
- [x] `_TEMPLATE-TELEMETRY-EVENT.json` válido (Draft 2020-12) commitado
- [x] README de contrato commitado com 3 exemplos canônicos
- [x] Fixtures em `src/test/resources/fixtures/telemetry/` commitadas
- [x] `.gitignore` atualizado para excluir `.claude/telemetry/index.json`
- [x] Pelo menos 1 teste automatizado de validação de schema (JSON Schema validator contra fixtures)
- [x] Smoke test: amostra gerada via `jq` valida contra o schema
...
- **Acceptance Criteria:**
  - [x] Schema é Draft 2020-12 válido (validator não aponta erros sintáticos)
  - [x] Todos os 5 campos obrigatórios estão em `required`
  - [x] Enum de `type` contém os 11 valores canônicos
...
- **Acceptance Criteria:**
  - [x] 3 fixtures válidas cobrindo `session.start`, `skill.end`, `tool.call` com `status=failed`
  - [x] 2 fixtures inválidas para os casos de erro/boundary
  - [x] Teste parametrizado itera todas as fixtures
...
- **Files:**
  ...
  - `.gitignore`
- **Acceptance Criteria:**
  - [x] README explica cada campo do schema e os 3 exemplos
  - [x] Seção "Storage Layout" cita os 2 paths canônicos e a política de gitignore
  - [x] `.gitignore` inclui `.claude/telemetry/index.json`
...
- **Acceptance Criteria:**
  - [x] Teste gera um evento via `jq` ou builder inline e valida contra o schema
  - [x] Duração do teste < 2s
