# História: Documentação, ADR e Release 3.8.0
...
Como **mantenedor**, eu quero documentação consolidada (ADR-0004, CLAUDE.md, `.claude/README.md`, CHANGELOG) e um release `3.8.0` via release branch seguindo Git Flow (Rule 09), publicando o épico como feature MINOR no `main`.

Esta story fecha o épico. ADR-0004 documenta a arquitetura de telemetria (captura híbrida, storage per-epic, scrubbing). CLAUDE.md root e `.claude/README.md` ganham seção "Telemetria" com visão executiva e instruções de uso. CHANGELOG.md recebe entrada sob `[3.8.0] - YYYY-MM-DD` com seções Added/Changed.
...
1. Criar branch `release/3.8.0` a partir de `develop` quando todas as stories estiverem merged
2. Bump version `3.7.0-SNAPSHOT` → `3.8.0` em `pom.xml` + `CLAUDE.md` root se aplicável
3. Atualizar CHANGELOG com a data real
4. PR `release/3.8.0` → `main` (merge commit, não squash)
5. Tag `v3.8.0` no `main`
6. Back-merge `release/3.8.0` → `develop` (merge commit)
7. Bump `develop` para `3.9.0-SNAPSHOT`

## 3.5 Entrega de Valor

- **Valor Principal:** Release 3.8.0 publicado com feature completa; usuários externos recebem telemetria sem configuração manual.
- **Métrica de Sucesso:** Tag `v3.8.0` no `main`; CHANGELOG atualizado; ADR-0004 linkado de CLAUDE.md.
...
- [ ] `pom.xml` bumped para `3.8.0`
- [ ] Tag `v3.8.0` no `main`
- [ ] Back-merge em `develop` concluído com bump `3.9.0-SNAPSHOT`
...
| CHANGELOG atualizado | Entry `[3.8.0]` existe |
| Versão bumped | `pom.xml` tem `3.8.0` |
...
sequenceDiagram
    ...
    participant Rel as release/3.8.0
    participant Main as main
    participant Tag as tag v3.8.0
...
Cenario: Release flow completo (happy path)
  ...
  QUANDO executamos release/3.8.0 flow
  ENTÃO tag v3.8.0 existe no main
  E develop tem version 3.9.0-SNAPSHOT
  E CHANGELOG tem entrada [3.8.0] com data real
...
Cenario: CHANGELOG sem entrada 3.8.0 bloqueia (error path)
  DADO CHANGELOG sem entrada [3.8.0]
  QUANDO release script valida
  ENTÃO aborta com exit code 1

Cenario: Smoke do jar release (boundary)
  DADO jar 3.8.0 empacotado
...
- **Size:** M
- **Dependencies:** —
- **Branch:** `feature/task-0040-0012-001-adr`
...
- **Branch:** `feature/task-0040-0012-002-claude-readme`
- **Testability:** Config + VerificationTest
- **Files:**
...
### TASK-0040-0012-003: CHANGELOG.md entry 3.8.0

- **Layer:** Doc
- **Test Type:** Verification
- **Size:** S
- **Dependencies:** —
- **Branch:** `feature/task-0040-0012-003-changelog`
- **Testability:** Config + VerificationTest
- **Files:**
  - `CHANGELOG.md`
  - `java/src/test/java/dev/iadev/docs/ChangelogTest.java`
- **Acceptance Criteria:**
  - [ ] Entry `[3.8.0]` com Added e Changed
...
- **Branch:** `release/3.8.0` → `develop`
- **Testability:** Config + VerificationTest
- **Files:**
  - `pom.xml` (bump para 3.9.0-SNAPSHOT)
- **Acceptance Criteria:**
  - [ ] `develop` tem version `3.9.0-SNAPSHOT`
...
- **Size:** S
- **Dependencies:** TASK-0040-0012-005
- **Branch:** `feature/task-0040-0012-007-smoke`
