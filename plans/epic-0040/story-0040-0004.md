...
- [x] Story-0040-0003 concluída (scripts existem e são testáveis)
- [x] `SettingsAssembler` atual lido e compreendido (padrão de inserção)
- [x] `ProjectConfig` atual revisado

### DoD Local (Definition of Done)

- [x] `SettingsAssembler` injeta 5 hooks quando `telemetryEnabled=true`
- [x] Coexistência com `post-compile-check.sh` no `PostToolUse` validada (ambos rodam)
- [x] `HooksAssembler` copia 7 arquivos com chmod 0755
- [x] `ProjectConfig.telemetryEnabled` com default `true` e testes
- [x] Golden file regenerado e commitado
- [x] `SettingsAssemblerTelemetryTest` com cobertura ≥ 95%
- [x] Smoke test: executar `mvn process-resources` em projeto-teste → `.claude/settings.json` contém todos os hooks
...
- **Branch:** `feature/task-0040-0004-002-hooks-assembler`
...
- **Acceptance Criteria:**
  - [x] 7 arquivos copiados quando `telemetryEnabled=true`
  - [x] Permissões 0755 aplicadas em Unix
  - [x] Zero arquivos copiados quando `telemetryEnabled=false`
...
- **Size:** L
- **Dependencies:** TASK-0040-0004-001
- **Branch:** `feature/task-0040-0004-003-settings-merge`
- **Testability:** UseCase + AT
- **Files:**
...
- **Acceptance Criteria:**
  - [x] 5 eventos de hook injetados com matcher correto
  - [x] Coexistência com `post-compile-check.sh` preservada (2 entries no PostToolUse)
  - [x] Cobertura ≥ 95% line
...
- **Branch:** `feature/task-0040-0004-004-golden-regen`
- **Testability:** Config + VerificationTest
- **Files:**
  - `java/src/test/resources/golden/claude/settings.json`
  - `java/src/test/java/dev/iadev/golden/SettingsGoldenTest.java`
- **Acceptance Criteria:**
  - [x] Golden file atualizado via `GoldenFileRegenerator`
  - [x] Teste de comparação passa byte-a-byte
...
- **Branch:** `feature/task-0040-0004-005-docs-smoke`
- **Testability:** Migration + Smoke
- **Files:**
  - `docs/configuration/project-config.md`
  - `java/src/test/java/dev/iadev/application/assembler/ProcessResourcesSmokeIT.java`
- **Acceptance Criteria:**
  - [x] Doc explica `telemetryEnabled` com default e exemplos
  - [x] Smoke test rodando em sandbox-dir reproduz pipeline completa em < 10s
