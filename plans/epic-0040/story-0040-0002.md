...
- [x] Schema da story-0040-0001 publicado e validado
- [x] Decisão sobre Jackson (vs. Gson) confirmada — Jackson é o default do projeto
- [x] Fixtures de telemetria disponíveis (`src/test/resources/fixtures/telemetry/`)

### DoD Local (Definition of Done)

- [x] Record `TelemetryEvent` criado com todos os campos do schema
- [x] Enums `EventType`, `EventStatus` criados
- [x] `TelemetryWriter` escreve NDJSON append-only thread-safe (testado)
- [x] `TelemetryReader` itera eventos sem carregar tudo em memória
- [x] `ExecutionState.telemetryPath` adicionado com retrocompatibilidade
- [x] Cobertura ≥ 95% line, ≥ 90% branch (bundle gate verde; pacote telemetria 93% line / 97% branch — resto são catches defensivos de IOException)
- [x] Pelo menos 1 teste de concorrência (2 threads escrevendo no mesmo arquivo)
- [x] Smoke test: 1000 eventos → ler via Reader → contar equivale a 1000
...
- **Size:** M
- **Dependencies:** —
- **Branch:** `feature/task-0040-0002-001-event-record`
...
- **Acceptance Criteria:**
  - [x] Record rejeita null em campos obrigatórios
  - [x] `toJsonLine()` produz NDJSON válido contra schema da story-0040-0001
  - [x] Cobertura do record ≥ 95% line
...
- **Branch:** `feature/task-0040-0002-002-writer`
...
- **Acceptance Criteria:**
  - [x] Writer escreve 1 linha por evento
  - [x] Teste concorrente com 2 threads produz 2 linhas válidas
  - [x] Lock timeout levanta `TelemetryWriteTimeoutException`
...
- **Size:** M
- **Dependencies:** TASK-0040-0002-001
- **Branch:** `feature/task-0040-0002-003-reader`
- **Testability:** Port + Adapter + IT
- **Files:**
  - `java/src/main/java/dev/iadev/telemetry/TelemetryReader.java`
  - `java/src/test/java/dev/iadev/telemetry/TelemetryReaderIT.java`
- **Acceptance Criteria:**
  - [x] Reader itera lazy (não carrega tudo em memória)
  - [x] `count()` funciona em arquivo vazio (retorna 0)
  - [x] `skipInvalid=true` ignora linhas corrompidas e loga warning
...
- **Size:** S
- **Dependencies:** TASK-0040-0002-001
- **Branch:** `feature/task-0040-0002-004-execstate-integration`
- **Testability:** Domain + UnitTest
- **Files:**
...
- **Acceptance Criteria:**
  - [x] Campo `telemetryPath` (nullable String + `telemetryPathOptional()` accessor) adicionado
  - [x] Teste carrega JSON v1 sem o campo e não quebra
  - [x] Teste carrega JSON v2 com o campo e popula corretamente
...
- **Branch:** `feature/task-0040-0002-005-smoke`
- **Testability:** Migration + Smoke
- **Files:**
  - `java/src/test/java/dev/iadev/telemetry/TelemetrySmokeTest.java`
- **Acceptance Criteria:**
  - [x] 1000 eventos escritos e lidos de volta produzem contagem idêntica
  - [x] Duração total < 2s
