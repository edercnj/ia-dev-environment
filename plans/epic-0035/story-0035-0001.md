...
- **Acceptance Criteria:**
  - [x] Frontmatter tem novo `description`, `allowed-tools` com `Skill, AskUserQuestion`, `argument-hint` com 4 flags novas
  - [x] Parameters table tem 4 linhas novas sem remover existentes
  - [x] `ReleaseSkillTest` existente continua verde
...
- **Acceptance Criteria:**
  - [x] `state-file-schema.md` tem schema completo com 14 phases
  - [x] Tem exemplo JSON canonical
  - [x] Tem seção "Transições Válidas"
  - [x] SKILL.md tem seção "Step 0 — Resume Detection"
  - [x] Step 0 verifica `gh`, `jq`, `gh auth status`
...
- **Acceptance Criteria:**
  - [x] `mvn process-resources` executado (fix para cache stale)
  - [x] Golden files regenerados via `GoldenFileRegenerator`
  - [x] `mvn test` verde (5372 tests, 0 falhas)
  - [x] `PlatformDirectorySmokeTest` verde
