...
# Build PR body and create PR — capture URL from gh pr create output
PR_URL=$(gh pr create \
  --base main \
...
if [ -z "$PR_URL" ]; then
  echo "ABORT [PR_CREATE_FAILED]: could not capture PR URL from gh pr create"
  exit 1
fi

# Derive PR number from the URL returned by gh (avoids ambiguity of gh pr view)
PR_NUMBER="${PR_URL##*/}"
...
- [x] Step 7 antigo removido
- [x] Phase OPEN-RELEASE-PR documentada com bash completo
- [x] PR body inclui CHANGELOG entry + instruções de resume
- [x] `prNumber`/`prUrl`/`prTitle`/`changelogEntry` persistidos no state file
- [x] Step 10 PUBLISH atualizado (só tag e release branch)
- [x] Flag `--skip-review` adicionada
- [x] Error code `PR_NO_CHANGELOG_ENTRY` na tabela
- [x] Teste `ReleaseOpenPrTest`
- [x] Golden files regenerados
- [x] `mvn verify -Pall-tests` verde (exceto PipelineSmokeTest pré-existente — 34 falhas idênticas ao baseline do branch story-0035-0001, unrelated ao escopo desta story)
...
- **Size:** M
- **Dependencies:** —
- **Branch:** `feature/task-0035-0003-001-open-pr-structure`
- **Testability:** Config + VerificationTest
- **Files:**
  - `java/src/main/resources/targets/claude/skills/core/x-release/SKILL.md`
- **Acceptance Criteria:**
  - [x] Step 7 antigo removido
  - [x] Phase OPEN-RELEASE-PR documentada
  - [x] PR body template documentado
  - [x] Error codes na tabela
...
- **Branch:** `feature/task-0035-0003-002-skip-review`
- **Testability:** Config + VerificationTest
- **Files:**
  - `java/src/main/resources/targets/claude/skills/core/x-release/SKILL.md`
- **Acceptance Criteria:**
  - [x] Step 10 remove push main/develop
  - [x] Flag `--skip-review` documentada
  - [x] Integração x-review-pr fire-and-forget
...
- **Branch:** `feature/task-0035-0003-003-open-pr-tests`
...
- **Acceptance Criteria:**
  - [x] `ReleaseOpenPrTest` valida ausência de `git merge main`
  - [x] Valida presença de `gh pr create --base main`
  - [x] Golden files regenerados
  - [x] `mvn verify -Pall-tests` verde (baseline do story-0035-0001 preservado)
