...
if [ $MERGE_EXIT -eq 1 ]; then
  ...
  # the conflict and the reviewer resolves it in the PR UI (or locally on the branch).
  ...
  # The PR against develop will show the conflicting files so the reviewer
  # can resolve them inline in GitHub UI or by checking out the branch.
  git push -u origin "main:refs/heads/${BACKMERGE_BRANCH}"

  # Serialize conflict list into a JSON array for jq (one file per line).
  CONFLICTS_JSON=$(printf '%s\n' "$CONFLICT_LIST" | jq -R -s 'split("\n") | map(select(length > 0))')
...
Java SNAPSHOT advance was NOT applied — add it manually during conflict resolution if needed."
  ...
  jq --argjson files "$CONFLICTS_JSON" \
    '.phase = "BACKMERGE_CONFLICT" | .conflictFiles = $files | .phasesCompleted += ["BACK_MERGE_DEVELOP_CONFLICT"]' \
    plans/release-state-${VERSION}.json > tmp && mv tmp plans/release-state-${VERSION}.json
fi
```
...
- **Size:** M
- **Dependencies:** —
- **Branch:** `feature/task-0035-0006-001-backmerge-clean`
...
- **Branch:** `feature/task-0035-0006-002-backmerge-conflict`
- **Testability:** Port + Adapter + IT
- **Files:**
  - `java/src/main/resources/targets/claude/skills/core/x-release/SKILL.md`
...
- **Branch:** `feature/task-0035-0006-003-backmerge-tests`
