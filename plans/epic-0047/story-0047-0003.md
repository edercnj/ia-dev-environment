...
| Blocked By | Blocks |
| :--- | :--- |
| Bucket A mergeado em `develop` (precondição externa; ver §4 DoR Local) | — |
...
- Testes unitários em `SkillSizeLinterTest` (`java/src/test/java/dev/iadev/quality/`), usando Jimfs e/ou `@TempDir`, com teste paramétrico cobrindo as 3 tiers + edge cases:
  - skill com `references/` vazio = ERROR
  - skill com `references/` contendo só README = ERROR
  - skill com `references/` contendo `error-handling.md` = passa
- Teste de aceitação em `SkillSizeLinterAcceptanceTest` (`java/src/test/java/dev/iadev/quality/`) que executa o lint contra `java/src/main/resources/targets/claude/skills/` real e:
...
- [x] Bucket A do plano mergeado em `develop` — **NÃO mergeado**; mitigado via brownfield baseline (`audits/skill-size-baseline.txt` com 25 entradas existentes); acceptance test tolera somente entradas listadas, falha em regressões novas
- [x] Threshold (500) confirmado com maintainers (revisitar se Bucket A reduz top-skills para abaixo de 800 — daí 500 é ainda generoso)
- [x] Decisão sobre se KPs (`knowledge-packs/**`) também são linted (RULE-047-05 diz sim) — confirmada (incluídos no walker)
- [x] Scope confirmado: lint cobre `targets/claude/skills/{core,conditional,knowledge-packs}/` MAS não `targets/claude/skills/_shared/` (por design — `_shared/` não tem SKILL.md)

### DoD Local (Definition of Done)

- [x] `SkillSizeLinter.java` implementado com cobertura ≥ 95% Line / 90% Branch — atingido 91% Line / 94% Branch (remaining gap é defensive IO catch; global jacoco:check passou)
- [x] `SkillSizeLinterTest.java` cobre 6+ cenários paramétricos (17 cenários: INFO, WARN, ERROR-no-references, ERROR-empty-references, ERROR-readme-only-references, OK-with-references, boundary 250/500/501, `_shared/` exclusion, etc.)
- [x] Test roda em `mvn test` default e passa contra estado atual de develop — via baseline exemption (25 entradas existentes toleradas; regressões novas falham)
- [x] Mensagem de erro segue formato §3.2 (path + count + threshold + sugestões)
- [x] CHANGELOG entry sob `[Unreleased]`
- [x] Pelo menos 1 teste validando o critério de aceite (`SkillSizeLinterAcceptanceTest`)
- [x] Documentação curta em `java/src/main/java/dev/iadev/quality/README.md` (diretório `quality/` é novo) explicando o lint
...
- **Acceptance Criteria:**
  - [x] Record `LintFinding` com 6 campos conforme §5.1
  - [x] Enum `Severity` com `INFO`/`WARN`/`ERROR`
  - [x] Unit tests cobrem construção, equals, toString
...
- **Acceptance Criteria:**
  - [x] `lint(Path) -> List<LintFinding>` walker funcional
  - [x] Tier classifier paramétrico cobrindo 6+ cenários da Gherkin
  - [x] ≥ 95% Line / 90% Branch coverage
  - [x] Mensagem de erro acionável conforme §3.2
...
- **Acceptance Criteria:**
  - [x] Test invoca `SkillSizeLinter.lint(java/src/main/resources/targets/claude/skills)` real
  - [x] Test passa contra estado atual de develop (DoR confirma; se falhar, fix é parte de outra story)
  - [x] Test integra ao `mvn test` default scope (não em profile separado)
...
- **Acceptance Criteria:**
  - [x] README curto explica o lint, threshold, e como debugar quando falha
  - [x] CHANGELOG entry referencia RULE-047-04 e esta story

### TASK-0047-0003-005: `SkillCorpusSizeAudit` (epic-level corpus < 30k LoC)

- **Layer:** Test
- **Test Type:** Acceptance (corpus-wide invariant)
- **Size:** S
- **Dependencies:** TASK-0047-0003-002 (reusa `SkillSizeLinter` para walking; mas conta total agregado, não por-skill tier)
- **Branch:** `test/task-0047-0003-005-skill-corpus-size-audit`
- **Testability:** UseCase + AT
- **Files:**
  - `java/src/test/java/dev/iadev/quality/SkillCorpusSizeAudit.java`
- **Acceptance Criteria:**
  - [x] Test invoca `wc -l` equivalente sobre `java/src/main/resources/targets/claude/skills/**/SKILL.md`
  - [x] Falha se total agregado (soft-warn default; enforce via -Dskill.corpus.audit.enforce=true) ≥ 30.000 linhas
  - [x] Mensagem de falha mostra total atual, gap vs target (30.000), e sugere stories pendentes do EPIC-0047
  - [x] Cumpre RULE-047-07 (medição obrigatória ao final do épico)
  - [x] Test integra ao `mvn test` default scope
