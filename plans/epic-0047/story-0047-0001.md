...
| Blocked By | Blocks |
| :--- | :--- |
| Bucket A mergeado em `develop`; Sprint 2 medição completada (precondições externas; ver §4 DoR Local) | story-0047-0002, story-0047-0004 |
...
- Se a opção escolhida for (a) placeholder, atualizar `SkillsAssembler.copyCoreSkill` ou `CopyHelpers.replacePlaceholdersInDir(...)` (delegando para `CopyTreeWalker`) para resolver `{{INCLUDE:...}}` antes do template substitution.
...
- [x] `java/src/main/resources/targets/claude/skills/_shared/` criado com 3 arquivos snippet + README
- [x] `adr/ADR-0006-shared-snippets-inclusion-strategy.md` mergeado com status `Accepted` — ver nota de numeração: entregue como `ADR-0011-shared-snippets-inclusion-strategy.md` porque o slot ADR-0006 já estava ocupado por EPIC-0041
- [x] Cluster pre-commit (`x-git-commit`, `x-code-format`, `x-code-lint`) consome o snippet via opção escolhida em ADR-0011 (Opção b — link Markdown relativo)
- [x] Goldens dos 17 perfis regenerados via `mvn process-resources` (sem regressões em outras skills — `mvn verify` PASS)
- [N/A] Se ADR-0006 escolheu opção (a): unit test `SnippetIncluderTest` — ADR-0011 escolheu Opção (b), sem `SnippetIncluder`; teste equivalente entregue: `SharedSnippetsAssemblerTest` (5 TPP) valida o copy do `_shared/` pelo assembler
- [x] Commit atômico por task (RULE-047-06) — 7 commits: TASK-001 doc, TASK-002 ADR, TASK-003 RED, TASK-003 GREEN, TASK-004 pilot+goldens, TASK-005 smoke, fix-FrontmatterSmoke exemption
- [x] Pelo menos 1 teste automatizado validando `_shared/` é copiado para output — `SharedSnippetsAssemblerTest` (5 testes)
- [x] Smoke: `Epic0047CompressionSmokeTest.smoke_sharedDirShipsToAllProfiles` valida presença em 17 perfis — 51 testes (3 métodos × 17 perfis)
