...
Como **engenheiro de release executando `x-release` para produzir uma nova versão**, eu quero a opção de aguardar o CI do release PR terminar antes da approval-gate humana, reduzindo o risco de criar tag na `main` em commit com CI amarelo ou pipeline de release falhando.
...
- **Métrica de Sucesso:** Invocação `x-release 3.7.0 --ci-watch` com CI amarelo no release PR falha a release com exit 20 antes de criar tag na `main`.
