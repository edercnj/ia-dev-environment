...
- Script bash em `scripts/audit-rule-20.sh` (ou equivalente integrado ao maven) que:
  - Grep por `x-pr-create` em `java/src/main/resources/targets/claude/skills/core/**/SKILL.md`
  - Para cada match, verifica se o mesmo arquivo invoca `x-pr-watch-ci` OU declara `--no-ci-watch`
