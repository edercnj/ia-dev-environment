...
Adicionar à `java/src/main/resources/targets/claude/skills/core/git/x-git-worktree/SKILL.md`, após "Operation 4: cleanup":
...
2. LIST      → `git worktree list --porcelain` (all known worktrees)
3. CLASSIFY  → If toplevel matches one of the listed worktrees AND is under `.claude/worktrees/`, set `inWorktree=true`; otherwise `false`.
