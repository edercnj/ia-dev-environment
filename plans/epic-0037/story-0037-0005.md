...
Editar `java/src/main/resources/targets/claude/skills/core/dev/x-story-implement/SKILL.md`:
...
6. **Branch Creation (worktree-aware):**
   ...
   **Step 6.2 — Decide branch creation strategy:**
   ...
   | :--- | :--- | :--- |
   | Standalone, in main repo | NOT passed | Legacy: `git checkout -b feat/story-XXXX-YYYY-desc` from `develop`. No worktree. (Backward compat) |
   | Standalone, in main repo | passed | Call `/x-git-worktree create --branch feat/story-XXXX-YYYY-desc --base develop --id story-XXXX-YYYY`. `cd` to returned path. Set `STORY_OWNS_WORKTREE=true` for cleanup tracking. |
   | Orchestrated (already in worktree) | (any) | Detected `IN_WT=true`. Reuse current cwd. Run `git checkout -b feat/story-XXXX-YYYY-desc` inside the existing worktree. Set `STORY_OWNS_WORKTREE=false`. **Do NOT** create nested. |
...
Cenario: Backward compat — standalone sem --worktree
  ...
  ENTÃO Phase 0 Step 6 executa git checkout -b feat/story-0037-0005-... no cwd atual
...
Cenario: Orquestrada por epic-implement (já em worktree)
  ...
  E git checkout -b feat/story-0037-0005-... é executado no cwd atual
  E STORY_OWNS_WORKTREE=false
