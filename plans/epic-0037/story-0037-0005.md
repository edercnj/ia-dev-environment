# História: `x-dev-story-implement` Phase 0 Worktree-Aware
...
Como **usuário ou orquestrador** invocando `x-dev-story-implement` standalone (fora de `x-dev-epic-implement`), eu quero a opção de criar um worktree isolado para a story via flag `--worktree`, permitindo rodar múltiplas stories em paralelo no mesmo repo sem conflito de branch. Quando invocada **dentro** de `x-dev-epic-implement` (que já criou um worktree na STORY 3), a detecção deve garantir que `x-dev-story-implement` **reusa** o worktree existente em vez de criar nested.
...
Editar `java/src/main/resources/targets/claude/skills/core/x-dev-story-implement/SKILL.md`:
...
Editar a Phase 3 do `x-dev-story-implement/SKILL.md` para adicionar cleanup condicional:
...
- Detection ensures zero nesting when invoked by `x-dev-epic-implement`.

## 3.6 Entrega de Valor

- **Valor Principal:** Stories implementadas standalone ganham isolamento opt-in. Quando o usuário roda 2 stories standalone em paralelo, cada uma fica em seu worktree, eliminando conflito de branch. Quando rodada via `x-dev-epic-implement`, a detecção garante zero nesting.
- **Métrica de Sucesso:** Smoke manual: 2 invocações concorrentes de `/x-dev-story-implement story-XXXX-YYYY --worktree` em terminais diferentes completam sem erro. Quando chamada como subagent de epic-implement, `STORY_OWNS_WORKTREE=false` e cleanup é skipped.
...
- [ ] `x-dev-story-implement/SKILL.md` Phase 0 e Phase 3 lidas integralmente
...
Cenario: Backward compat — standalone sem --worktree
  ...
  QUANDO executo /x-dev-story-implement story-0037-0005
...
Cenario: Standalone com --worktree em main repo
  DADO que estou no main checkout
  E passo --worktree
  QUANDO executo /x-dev-story-implement story-0037-0005 --worktree
...
Cenario: Orquestrada por epic-implement (já em worktree)
  ...
  QUANDO /x-dev-story-implement story-0037-0005 é invocado como subagent
...
Cenario: Boundary — duas invocações standalone paralelas com --worktree
  DADO que executo /x-dev-story-implement story-0037-0005 --worktree em terminal A
  E executo /x-dev-story-implement story-0037-0005-bis --worktree em terminal B
...
- **Files:**
  - `java/src/test/resources/golden/*/.claude/skills/x-dev-story-implement/SKILL.md`
- **Acceptance Criteria:**
