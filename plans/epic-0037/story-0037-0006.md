...
Editar `java/src/main/resources/targets/claude/skills/core/dev/x-task-implement/SKILL.md`:
...
Localizar o bloco de instruções do subagent que executa `git checkout -b feat/STORY-ID-short-description` no `x-task-implement/SKILL.md` (atualmente em torno da linha 150). Substituir por:
...
| :--- | :--- | :--- |
| Standalone, in main repo | NOT passed | Legacy: `git checkout -b feat/task-XXXX-YYYY-NNN-desc` from parent branch (story branch or `develop`). No worktree. |
| Standalone, in main repo | passed | Call `/x-git-worktree create --branch feat/task-XXXX-YYYY-NNN-desc --base <parent> --id task-XXXX-YYYY-NNN`. `cd` to returned path. Set `TASK_OWNS_WORKTREE=true`. |
| Inside story worktree (orchestrated) | (any) | Detected `IN_WT=true`. Reuse current cwd. Run `git checkout -b feat/task-XXXX-YYYY-NNN-desc` inside the existing worktree. Set `TASK_OWNS_WORKTREE=false`. **Do NOT** create nested. |
...
| Sem flag, mas a story tem branch parent existente (`feat/story-XXXX-YYYY-...`) | `feat/story-XXXX-YYYY-...` |
...
Cenario: Backward compat — standalone sem --worktree
  ...
  ENTÃO git checkout -b feat/task-0037-0006-001-... é executado no cwd atual
  E nenhum worktree é criado
  E TASK_OWNS_WORKTREE=false

Cenario: Standalone com --worktree, sem story parent
  DADO que estou no main checkout
  E não há branch feat/story-0037-0006-* ativa
...
Cenario: Standalone com --worktree, com story parent
  DADO que estou na branch feat/story-0037-0006-foundation
  QUANDO executo /x-task-implement task-0037-0006-001 --worktree
  ENTÃO base resolution retorna "feat/story-0037-0006-foundation"
...
Cenario: Nested prevention — task dentro de story worktree
  ...
  E git checkout -b feat/task-... executa no cwd atual (dentro da story worktree)
  E TASK_OWNS_WORKTREE=false
