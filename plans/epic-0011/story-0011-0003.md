# História: Implementar integração Jira no skill x-story-epic
...
Como **engenheiro de plataforma**, eu quero que o skill `x-story-epic` possua um Step 5.5 (Optional Jira Integration) que verifique a disponibilidade do MCP Jira, pergunte ao usuario se deseja criar o epic no Jira, execute a criação via MCP, capture a chave retornada e atualize o markdown gerado, para que epics criados pelo skill possam ser automaticamente sincronizados com o Jira.

### Contexto

O skill `x-story-epic` (`java/src/main/resources/skills-templates/core/x-story-epic/SKILL.md`) gera epics com stories associadas em formato markdown. Atualmente, não existe integração com ferramentas de gestão de projetos. Este step adicional deve ser completamente opcional e funcionar em dois modos:
...
- Chamar MCP para criação do epic com os campos definidos no contrato
...
- [ ] Contrato de dados da API Jira para criação de Epic definido
...
- [ ] Chave Jira preenchida no markdown após criação bem-sucedida
...
| `summary` | String | M | - | Título do epic extraido do header markdown |
| `description` | String | M | - | Texto da seção "Visao Geral" do epic |
...
| `cascadeToStories` | boolean | Não | Se deve propagar criação para stories |
| `epicIssueKey` | String | Não | Chave do epic (preenchida após criacao) |
...
sequenceDiagram
    ...
    S->>S: Step 5.5 - Verificar MCP disponível
...
```mermaid
flowchart TD
    A[Step 5.5 Start] --> B{MCP Jira disponível?}
...
Funcionalidade: Integracao Jira no skill x-story-epic
  ...
  Cenário: MCP não disponível resulta em skip silencioso
  ...
  Cenário: Usuario diz Sim e epic e criado no Jira com chave preenchida no markdown
    DADO que o skill x-story-epic esta sendo executado em modo standalone
    E o MCP Jira esta disponível no ambiente
    E o epic gerado possui título "Epic de Pagamentos" e descrição na seção Visão Geral
    ...
    E uma mensagem de confirmação deve ser exibida ao usuario

  Cenário: Usuario diz Não e apenas markdown e gerado sem chave Jira
    DADO que o skill x-story-epic esta sendo executado em modo standalone
    E o MCP Jira esta disponível no ambiente
    QUANDO o Step 5.5 pergunta ao usuario se deseja criar o epic no Jira
  ...
  Cenário: MCP falha ao criar epic resulta em warning e chave com mensagem de erro
    DADO que o skill x-story-epic esta sendo executado
    E o MCP Jira esta disponível no ambiente
    E o usuario autorizou a criação do epic no Jira
    ...
    E a execução do skill deve continuar normalmente sem interromper o fluxo

  Cenário: Invocacao cascaded com jiraContext presente pula prompt ao usuario
...
- [ ] **[Dev]** Adicionar Step 5.5 ao skill `x-story-epic/SKILL.md` com logica de verificação de MCP
- [ ] **[Dev]** Implementar deteccao de disponibilidade do MCP Jira via tool listing
- [ ] **[Dev]** Implementar fluxo standalone com `AskUserQuestion` para confirmação e project key
...
- [ ] **[Test]** Criar testes para cenario MCP indisponível (skip silencioso)
