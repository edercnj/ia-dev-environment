...
- **Telemetria (S12)**: linhas JSONL incluem campo `releaseType: "hotfix"`
...
- [ ] `--hotfix` herda auto-detect, prompts, smart resume, pre-flight, summary, telemetria
- [ ] Auto-detect rejeita feat/breaking com `HOTFIX_INVALID_COMMITS`
- [ ] State file separado (não conflita com release normal em curso)
- [ ] Diagrama summary mostra ramo de hotfix
- [ ] Telemetria com `releaseType: "hotfix"`
- [ ] Smoke valida fluxo completo de hotfix interativo
...
- Schema: idêntico ao v2 (S02) com novo campo `releaseType: "hotfix"` adicionado
...
- **Acceptance Criteria:**
  - [ ] `ReleaseContext.hotfix()` factory
  - [ ] `restrictBumpTo` configurável (PATCH para hotfix)
  - [ ] `baseBranch` parametrizável (`main` para hotfix)
...
- **Acceptance Criteria:**
  - [ ] Cada componente aceita `ReleaseContext`
  - [ ] Comportamentos hotfix/release são distintos e testados
...
- **Acceptance Criteria:**
  - [ ] Seção "Hotfix Flow" documenta diferenças
  - [ ] Erros HOTFIX_* no catalog
...
- **Acceptance Criteria:**
  - [ ] Fixture: tag v3.1.0 + 2 fix commits → hotfix v3.1.1 simulado completo
  - [ ] State separado, summary correto, telemetria com releaseType
