...
- **Acceptance Criteria:**
  - [x] Despacha N tasks em pool de tamanho `min(CPU,4)`
  - [x] Captura exit code + duração de cada
  - [x] Aggregator ordena por severidade
...
- **Acceptance Criteria:**
  - [x] Bloco bash usa `&` + `wait` para os 7 checks
  - [x] Captura e ordena failures
  - [x] Documenta `--max-parallel`
...
- **Acceptance Criteria:**
  - [x] Mede tempo sequencial vs paralelo num fixture
  - [x] Asserta redução ≥ 40%
...
- **Acceptance Criteria:**
  - [x] Force fail no golden check; valida abort com `VALIDATE_GOLDEN_DRIFT`
