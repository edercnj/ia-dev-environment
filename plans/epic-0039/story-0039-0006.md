...
- `--no-github-release`: pula sem perguntar
- `--auto-github-release`: cria sem perguntar (para automação total)
...
- [ ] Decisão sobre `--auto-github-release` ratificada
- [ ] Comportamento em rate-limit definido (warn-only)

### DoD Local

- [ ] Phase 11 PUBLISH sempre pergunta (interactive mode)
- [ ] `--no-github-release` e `--auto-github-release` funcionais e mutex
...
| :--- | :--- | :--- | :--- | :--- |
| `--no-github-release` | flag | O | mutex com `--auto-github-release` | `--no-github-release` |
| `--auto-github-release` | flag | O | mutex | `--auto-github-release` |
...
| 1 | `PUBLISH_FLAG_CONFLICT` | ambos `--no-github-release` e `--auto-github-release` passados |
...
sequenceDiagram
    ...
    alt --no-github-release
        Skill-->>Skill: skip
    else --auto-github-release
        Skill->>Gh: release create
    else interactive
...
Cenario: --auto-github-release cria sem prompt (boundary)
  QUANDO eu rodo /x-release --continue-after-merge --auto-github-release
  ENTÃO release é criado sem pergunta interativa
...
Cenario: Flags mutex — conflito (error)
  QUANDO eu rodo /x-release --no-github-release --auto-github-release
  ENTÃO exit 1 com PUBLISH_FLAG_CONFLICT
```

### 7.1 TPP Ordering

Degenerate (skip) → happy → boundary (auto) → error (gh fail, mutex).
...
- [x] Boundary: --auto-github-release
