...
- Lê `execution-state.json` → `planning_schema_version`
...
- Após todas as phases completarem:
  - Verificar `execution-state.json`: todas as stories têm status DONE
...
- Atualizar `execution-state.json` → `epic.status = "DONE"` + finished_at
- Gerar `plans/epic-XXXX/reports/epic-execution-report.md` (summary)
- Retornar estrutura `{status, stories_done, phases_executed, coverage_delta, prs_opened}`
...
```json
{
  ...
  "status": "IN_PROGRESS",
  "stories": {
    "STORY-0039-0001": { "status": "DONE", "pr_url": "..." },
    "STORY-0039-0002": { "status": "PENDING" }
  }
}
```
...
| Seção | Conteúdo |
| :--- | :--- |
| `## Epic Summary` | epic_id, status, schema_version, started/finished |
...
| :--- | :--- | :--- |
| `epic_id` | `String` | ID do épico |
| `status` | `Enum(DONE\|FAILED\|PARTIAL)` | Resultado final |
| `stories_done` | `List[String]` | IDs das stories concluídas |
| `phases_executed` | `Integer` | Número de phases completas |
| `coverage_delta` | `{line, branch}` | Δ agregado |
| `prs_opened` | `List[String]` | URLs dos PRs |
| `wallclock_ms` | `Integer` | Tempo total |
...
sequenceDiagram
    ...
    EI->>ES: read planning_schema_version (2.0)
    ...
    EI->>ES: update epic status DONE
    EI-->>U: epic-execution-report.md + {stories_done, prs}
```
...
Cenario: Error — story falha aborta phase
  ...
  E S001 permanece com status DONE (preservada)
  E phase 2 nunca é iniciada

Cenario: Error — epic verification falha após todas stories DONE
  DADO que todas as 4 stories concluem com status DONE
...
Cenario: Boundary — schema v1 legacy (backward compat)
  DADO que execution-state.json não declara planning_schema_version (épico 0034 legacy)
  QUANDO x-epic-implement é invocada
