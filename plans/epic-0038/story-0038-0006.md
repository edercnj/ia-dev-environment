...
- Após todas as waves completarem:
  - Verificar `execution-state.json`: todas as tasks da story têm status DONE
...
- Atualizar `execution-state.json` → `stories[STORY-ID].status = "DONE"` + PR number
- Retornar estrutura `{status, pr_url, tasks_done, coverage_delta, wallclock_ms}` ao caller (x-epic-implement)
...
```json
{
  ...
  "stories": {
    "STORY-0039-0001": {
      "status": "DONE",
      "pr_url": "https://github.com/.../pull/345",
      "started_at": "2026-04-13T10:00:00Z",
      "finished_at": "2026-04-13T10:25:00Z",
      "waves_executed": 2,
      "tasks_done": ["TASK-0039-0001-001", "TASK-0039-0001-002", "TASK-0039-0001-003"]
    }
  }
}
```
...
sequenceDiagram
    ...
    PR-->>SI: pr_url
    SI->>ES: update story DONE + pr_url
    SI-->>EI: {DONE, pr_url, tasks_done, coverage}
```
...
Cenario: Happy path — story com 2 waves executa e abre PR
  ...
  E execution-state.json mostra story status DONE com pr_url
...
- **Acceptance Criteria:**
  ...
  - [ ] execution-state.json atualizado com pr_url
