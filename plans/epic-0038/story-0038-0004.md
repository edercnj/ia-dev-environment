...
O comportamento legacy (v1) é **integralmente preservado** quando `execution-state.json` declara `planningSchemaVersion == "1.0"` (ou quando o flag está ausente). A skill detecta a versão no início e escolhe o flow: v1 → fluxo monolítico atual; v2 → fluxo task-first com as 5 phases descritas em §5.3 do spec. Isso é crítico porque o próprio EPIC-0038 roda em v1 durante sua execução (bootstrap).

### 3.1 Detecção de Schema Version

- Ler `plans/epic-XXXX/execution-state.json` no início de `x-story-plan`
- Se `planningSchemaVersion == "2.0"` → flow v2 (task-first)
- Se `planningSchemaVersion == "1.0"`, ausente, malformado, ou valor não reconhecido (ex.: `"3.0"`, `"legacy"`) → flow v1 (fallback soft, sem hard fail). Warning log emitido conforme matriz autoritativa em story-0038-0008 §5.2 (`SCHEMA_VERSION_FALLBACK_NO_FILE`, `SCHEMA_VERSION_FALLBACK_MISSING_FIELD`, `SCHEMA_VERSION_INVALID_VALUE`).
- `x-story-plan` NÃO duplica a lógica de detecção: invoca o resolver compartilhado entregue por story-0038-0008 e respeita a decisão. Hard fail só ocorre se o próprio `execution-state.json` for JSON inválido (não parseável).
...
- Escrever `task-TASK-XXXX-YYYY-NNN.md` per task. O **schema formal** (estrutura de seções e validação) é definido em story-0038-0001; o **template físico distribuído** (`shared/templates/_TEMPLATE-TASK.md` com placeholders `{{TASK_ID}}` etc.) é entregue em story-0038-0009. Bootstrap: até 0009 mergear, esta story usa um template inline equivalente ao schema da 0001 (mesmas seções obrigatórias) — aceitável porque o épico inteiro roda em v1, então 0004 não executa a Fase 2 "for real" antes de 0009 estar disponível.
...
- [ ] Execution-state schema com `planningSchemaVersion` confirmado em story-0038-0008
- [ ] Branch `feature/story-0038-0004-story-plan-invokes-task-plan` criada a partir de `develop`

### DoD Local

- [ ] `x-story-plan/SKILL.md` refatorada com detecção de `planningSchemaVersion`
...
| `--force-legacy` | `Flag` | O | Força flow v1 ignorando `planningSchemaVersion` (escape hatch de debug) |
...
> **Schema alinhado:** chaves em camelCase, enum `status` ∈ `PENDING | IN_PROGRESS | SUCCESS | DEFERRED | FAILED`. Idêntico ao formato canônico em uso.
...
| :--- | :--- | :--- |
| `SCHEMA_VERSION_UNKNOWN` | `planningSchemaVersion` não é "1.0" ou "2.0" | "Unsupported planningSchemaVersion: {v}" |
...
sequenceDiagram
    ...
    SP->>ES: read planningSchemaVersion
    ES-->>SP: "2.0"
...
```mermaid
flowchart TD
    START[x-story-plan invoked] --> DETECT{planningSchemaVersion}
...
```gherkin
Cenario: Degenerate — execution-state sem planningSchemaVersion
  DADO que plans/epic-0038/execution-state.json não contém o campo planningSchemaVersion
...
Cenario: Error — planningSchemaVersion não reconhecido
  DADO que execution-state.json declara planningSchemaVersion: "3.0"
  QUANDO x-story-plan é invocada
...
### TASK-0038-0004-001: Detecção de `planningSchemaVersion` + branching
...
- **Acceptance Criteria:**
  - [ ] Story com `planningSchemaVersion: "1.0"` produz output idêntico ao baseline
