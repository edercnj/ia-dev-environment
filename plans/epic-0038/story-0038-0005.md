...
- Retornar estrutura `{status, sha, coverage_delta, wallclock_ms}` ao caller (x-story-implement)
...
- [ ] `planning_schema_version` detectable em `execution-state.json` (story-0038-0008 não precisa estar mergeada, mas a feature flag deve existir)
...
| `coverage_delta` | `{line: float, branch: float}` | Sim | Δ de cobertura vs pré-execution |
| `wallclock_ms` | `Integer` | Sim | Tempo total de execução |
...
sequenceDiagram
    ...
    TI-->>SI: {status: DONE, sha, coverage_delta, wallclock}
```
...
Cenario: Boundary — schema v1 legacy path preservado
  DADO que execution-state.json não declara planning_schema_version
...
- **Acceptance Criteria:**
  - [ ] Roteia para v1 ou v2 baseado em planning_schema_version
