...
| :--- | :--- | :--- |
| 15 | `15-task-testability.md` | RULE-TF-01 — toda task declara: `independently testable` OU `requires mock of TASK-XXX` OU `coalescible with TASK-ZZZ`. Sem declaração → commit bloqueado por `x-task-implement`. |
| 16 | `16-task-io-contracts.md` | RULE-TF-02 — inputs (pré-condições) e outputs (pós-condições) obrigatórios por task. Outputs verificáveis via grep/assert/teste. Template `_TEMPLATE-TASK.md` exige as seções. |
| 17 | `17-topological-execution.md` | RULE-TF-03 — ordem de execução respeita `task-implementation-map-STORY-*.md`. Paralelismo opt-in via análise automática. `x-story-implement` falha se map ausente (schema v2). |
| 18 | `18-task-atomic-commits.md` | RULE-TF-04 — 1 task = 1 commit (Conventional Commits com escopo `task(TASK-XXXX-YYYY-NNN)`). Coalesced groups: 1 commit + footer `Coalesces-with: TASK-..., TASK-...`. |
| 19 | `19-planning-schema-backward-compat.md` | RULE-TF-05 — épicos com `planning_schema_version == "1.0"` (ou ausente) executam via legacy loader. Novos épicos usam `"2.0"`. Smoke test em 0038-0008 é a evidência. |

Cada rule file segue a estrutura canônica (ver referência `14-worktree-lifecycle.md`): frontmatter com escopo/ownership, seção Enforcement, seção Anti-Patterns, cross-refs para skills afetadas.
...
- Atualizar contagem total de rules (atualmente 10; sobe para 15 — sem contar slots 10/11/12 reservados).
...
> **Escopo:** Todo `task-TASK-NNN.md` em épicos com `planning_schema_version == "2.0"`.
> **Ownership:** Platform Team.
...
```mermaid
graph TD
    R15[RULE-TF-01<br/>15-task-testability] -->|enforced by| SkillTP[x-task-plan]
    R15 -->|enforced by| SkillTI[x-task-implement]
    R16[RULE-TF-02<br/>16-task-io-contracts] -->|enforced by| SkillTP
    R17[RULE-TF-03<br/>17-topological-execution] -->|enforced by| SkillSI[x-story-implement]
    R18[RULE-TF-04<br/>18-task-atomic-commits] -->|enforced by| SkillTI
    R18 -->|hook| PreCommit[pre-commit hook]
    R19[RULE-TF-05<br/>19-planning-schema-backward-compat] -->|enforced by| SkillEI[x-epic-implement]
```
...
### TASK-0038-0009-001: Criar Rule File `15-task-testability.md`
...
- **Files:**
  - `java/src/main/resources/targets/claude/rules/15-task-testability.md` (novo)
- **Acceptance Criteria:**
...
- **Testability:** Config + VerificationTest
- **Files:**
  - `targets/claude/rules/16-task-io-contracts.md` (novo)
  - `targets/claude/rules/17-topological-execution.md` (novo)
  - `targets/claude/rules/18-task-atomic-commits.md` (novo)
  - `targets/claude/rules/19-planning-schema-backward-compat.md` (novo)
- **Acceptance Criteria:**
