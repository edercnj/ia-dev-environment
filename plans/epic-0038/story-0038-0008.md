# História: Migration Path e Backward Compatibility via `planning_schema_version`
...
### 3.1 Flag `planning_schema_version` em `execution-state.json`

- Adicionar campo `planning_schema_version` no schema do arquivo `plans/epic-XXXX/execution-state.json` em nível raiz.
...
- **Métrica de Sucesso:** (a) Smoke test `PlanningSchemaBackwardCompatSmokeIT` verde executando epic-0029 legacy; (b) `mvn clean verify` verde no merge; (c) zero stories dos épicos 0025–0037 quebradas por falta de campo `planning_schema_version`.
...
- [ ] `planning_schema_version` adicionado ao schema do `execution-state.json` com validação JSON
...
| :--- | :--- | :--- | :--- | :--- |
| `planning_schema_version` | `String` (SemVer-like) | O | Enum: `"1.0"`, `"2.0"`. Ausente ou inválido → tratado como `"1.0"` com warning log | `"2.0"` |
| `epic_id` | `String` | M | Regex `epic-\d{4}` | `"epic-0038"` |
| `current_phase` | `String` | M | (existente, inalterado) | `"implementing"` |
| `stories` | `Array<Object>` | M | (existente, inalterado) | `[...]` |
...
| Campo `planning_schema_version` ausente | `"1.0"` | Warning log `SCHEMA_VERSION_FALLBACK_MISSING_FIELD` |
...
```json
{
  ...
  "stories": [
    {
      "story_id": "story-0039-0001",
      "task_implementation_map": "plans/epic-0039/plans/task-implementation-map-STORY-0039-0001.md",
      "waves": [ /* ... */ ]
    }
  ]
}
...
flowchart TD
    ...
    Exists -- Yes --> HasField{planning_schema_version present?}
...
Cenario: Happy path legacy — épico v1 executa sem regressão
  ...
  E execution-state.json sem o campo planning_schema_version
...
Cenario: Happy path task-first — novo épico v2
  DADO um épico hipotético com execution-state.json contendo "planning_schema_version": "2.0"
...
Cenario: Error — valor inválido degrada para legacy com warning
  DADO um execution-state.json com "planning_schema_version": "3.0"
  QUANDO x-epic-implement é invocado
  ENTÃO resolvePlanningSchemaVersion retorna "1.0"
...
Cenario: Boundary — campo presente mas vazio
  DADO um execution-state.json com "planning_schema_version": ""
...
### TASK-0038-0008-001: Adicionar Campo `planning_schema_version` ao Schema
