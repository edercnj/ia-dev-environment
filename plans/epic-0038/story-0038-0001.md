# História: Task como Artefato Primário (schema `task-TASK-XXXX-YYYY-NNN.md`)

> **Convenção de filename (canônica para todo o épico):** o pattern oficial é
> `task-TASK-XXXX-YYYY-NNN.md` (validado por regex `^task-TASK-\d{4}-\d{4}-\d{3}\.md$`),
> onde `XXXX-YYYY` é `epicId-storyOrdinal` e `NNN` é o ordinal da task.
> Exemplo concreto: `task-TASK-0038-0003-001.md`. O plano derivado segue
> `plan-task-TASK-XXXX-YYYY-NNN.md`. A forma curta `task-TASK-NNN.md` aparece
> em prosa como **shorthand documental** apenas — não é um pattern de filename
> aceito; o schema, o parser e os exemplos sempre usam o pattern completo.
...
### 3.1 Schema do arquivo `task-TASK-XXXX-YYYY-NNN.md`
...
- **Seção 5 — Plano de implementação:** placeholder (preenchido por `x-task-plan` em story-0038-0003 via `plan-task-TASK-XXXX-YYYY-NNN.md`). Nesta story é apenas referência: "ver `plan-task-TASK-XXXX-YYYY-NNN.md`".

### 3.2 Validador/parser

Implementar `TaskFileParser` (Java, domínio) capaz de:
- Ler um `task-TASK-XXXX-YYYY-NNN.md` e extrair: ID, story, status, inputs, outputs, testabilidade, dependências.
- Validar invariantes:
...
| `## 5. Plano de implementação` | Não (placeholder) | Referência a `plan-task-TASK-XXXX-YYYY-NNN.md` | — |
