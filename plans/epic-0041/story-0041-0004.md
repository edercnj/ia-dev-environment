...
- `--scope=epic`: deriva fases **primariamente** a partir das dependências declaradas nas stories (`Blocked By` / `Blocks`) via cálculo topológico, para evitar dependência circular com `x-epic-map` (que invoca `/x-parallel-eval` em seu Step 8.5 — ver story-0041-0005). Se um `implementation-map-XXXX.md` já existir E for passado explicitamente via `--map=<path>`, usa as fases declaradas no map como fonte canônica. Em nenhuma hipótese a skill tenta carregar `implementation-map-XXXX.md` implicitamente. Analisa pares dentro de cada fase; reporta colisões e sugere serialização **dentro da fase**.
